...
applications. \jhanote{something not clear: why are 2 and 3 problems?
  maybe ``need'' for workload management and support of various applications?}
There was a need to be able to harness the power of these
heterogeneous resources to run jobs. \pilotjobs have historically been
used as a means of solving this problem. We briefly discuss some of
...
availability. Although Master-Worker schemes were important, they
...
Another application that pre-dates pilot jobs \jhanote{please use the
  macro for \pilotjobs!!} is the BOINC middleware
system~\cite{Anderson:2004:BSP:1032646.1033223}. \jhanote{previous
  sentence is sloppy: application, \pilotjobs and middleware are all
  interchangably used in the same sentence!} \onote{I wouldn't call it a 'middleware system'. It's a volunteer-based, distributed master-worker framework} BOINC plays a significant
role in distributed computing acting as a Client/Server-based platform
\onote{don't use client/server-based. everything in dist. comp. is either client/server or p2p. it's not a very useful way to characterize the main aspect of a system}
which utilizes unused computer cycles on the client-side in order to
...
including personal computers was the first wide-use of the
Client/Server model \onote{I don't think so!} that essentially made a grid 
...
>>>>>>> b1cc5d610c79d628295c15637c20784535df0a9e
...
With AppLes, scheduling and resource-selection is handled by a
per-application AppleS agent.  In addition, a subset of \pilotjob functionality
such as application-scheduling are present in AppLeS.
...
multi-level scheduling and late-binding.  Such enhancements
