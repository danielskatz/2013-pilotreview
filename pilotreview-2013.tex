...
resources.\jhanote{one or more resources?}\mtnote{Done.} Performance of workload
...
that overload the term with two separate meanings: resource as what is held and
resource as DCR?}
...
characteristic. It weakens the punch of an impactful and important sentence.}
\mtnote{Any idea about what term would you like to use?}
\jhanote{Also in the subsequent sentence, ``The tasks of a workload..''. I've
...
happy to bring it back modified if you think it is needed.} The terms `pilot'
...
DCR middleware where such a container is submitted. \jhanote{maybe workload in
lieu of container in the previous sentence?}\mtnote{The idea is that job is
dependent from a middleware, specifically a HPC-based one. For example, we do
not submit a `job' to a cloud middleware. I replaced `executed' by `submitted'.
Any better?} Accordingly, tasks are considered as the functional units of a
workload, while jobs as a way to schedule tasks on a DCR with a specific
middleware. \jhanote{minor nitpic: are we assuming 1 task per job? I don't think
we should..} \mtnote{I do not think we are assuming a 1:1 relation as we use
`tasks'. Should we make it explicit?}  It should be noted that, given their
...
\jhanote{We start with jobs and tasks, but it gets a bit tricky to navigate this
  ...
  qualitatively jobs from tasks. Maybe the distinction is not convincing?}
...
lifetime.\jhanote{changed DCI to DCR. OK?}\jhanote{not sure what ``out of the
queue'' means? maybe say, ``.. become available when the job is active, and is
available...''} \mtnote{Better? I used `once... has become' to indicate that the
job sits within the queue for a certain amount of time.}
...
\jhanote{also I would not use the word exclusive} \mtnote{Why?}\jhanote{In our
  ...
  the DCR scheduler as other jobs of the DCR cannot use those resources.}
...
use lower case for second instance of ``pilots'' ? This would make it consistent
with job, task, workload as these are now simply similar elements of our
vocabulary?} \mtnote{This has opened the proverbial can of warms. Ongoing but will need revision.} A portion of the resources of a DCR is allocated to
...
workload. \jhanote{what is the ``confinements of a single workload?''}
\mtnote{Better?}
...
entities: jobs on DCR middleware, and tasks on pilots. \jhanote{are we also
trying to say that multi-level scheduling is for different ``entities'' at the
different levels? either way, we may consider making that
...
\vocab{Bold}.  \prop{Italic} is used to refer to the core \prop{Properties} just
...
\impterm{Typewriter} font.

\mtnote{Do we need this formatting devices? They are not always added to every
use of the terms and they make the text a bit `heavy' for me.}
...
\mtnote{Reference needed} and in few other scientific domains \mtnote{Reference
  needed}.
...
consists of four \jhanote{logical?} components: a TaskScheduler, an
...
% Execution model.
The execution model of DAINE can be summarized in four steps: 1. the user
submits one or more jobs to DCR by means of SubmitScript(s) to bootstrap one or
more WorkerAgent; 2. When ready, the WorkAgent(s) reports back to the
ApplicationManager; 3. tasks are scheduled by the TaskScheduler on the available
WorkerAgent(s); 4. after execution, WorkerAgents send the output of the
computation back to the ApplicationManager.
...
pools.  Developed by [...] its main goal was originally to include DCRs exposing
grid middleware, into Condor pools. \jhanote{I'm not sure the meaning of the
previous sentence is clear: does it mean (a) DCRs that expose grid middleware
into a pool, (b) although we don't define/use federate, does ``include''
actually refer to federate?} \mtnote{I would not think so. A condor pool is a
list of DCRs with a set of properties for each item of the list. I added a
comma, better? That sentence needs to convey the following: there is a condor
pool, there are DCRs with a grid middleware (i.e. Globus), these DCRs need to be
added to the condor pool; The HTCondor Glidein system does that.}
...
Startd daemons, a Collector, and a Negotiator. Schedd is a queuing system that
...
HTCondor Glidein has been complemented by GlideinWMS, a Glidein-based workload
management system that automates deployment and management of glideins on
multiple types of DCR middleware. GlideinWMS builds upon the HTCondor Glidein
system by adding the following components: a set of Glidein Factory daemons, a
set of VO Frontend daemons, and a Collector dedicated to the WMS. Glidein
Factories submits tasks to the DCRs middleware, each VO Frontend matches the
tasks on one or more Schedd to the resource attributes advertised by a specific
Glidein Factory, and the WMS Collector holds references to all the active
Glidein Factories and VO Frontend daemons.
...
ClassAds, or is there another level of match-makeing for the glideins? There is
lack of documentation but I assume the Collector is the component where the
matchmaking function is implemented. ClassAds should be the name given to the
entries published by the Glidein Factories within the WMS collector (in the case
of GlideinWMS but it should be the same with the old HTCondor Glidein)} As such,
both pilot systems allow for late binding. Early binding is not available.
...
% Why it is a pilot system.
Figure~\ref{fig:diane_comparison} shows how PANDA implements the components and
functionalities of a pilot system as described in~\S\ref{sec:understanding}: the
...
submitted pilot(s) becomes available and reports back to the Job Dispatcher; 7.
tasks are dispatched to the available pilots for execution.
...
production sites. Other scheduler unable AutoPilot to submit to local and remote
batch systems or to the GlideinWMS frontend. Submissions via the canonical tools
offered by HTCondor have also been used to submit tasks to cloud resources via
\panda.

% Workload Semantics.
\panda has been designed to serve specifically the ATLAS use case and, as such,
to execute mostly single-core tasks with input and output files. Since its
...
with or without data dependences.
...
using BigJob~\cite{luckow2010}, an initial prototype of \pilot system.

% Architecture.
RADICAL-Pilot consists of three main components: a Pilot Manager, a Compute Unit
(CU) Manager, and a set of Agents. The Pilot Manager allows to describe pilots
and then submit them to DCR, while The CU manager to describe tasks (i.e. CU)
and to schedule them to one or more pilots. Agents are instantiated on DCRs and
execute the CUs pushed by the CU manager.
...
in~\S\ref{sec:understanding}. Clearly, the Pilot Manager is implemented and the
Workload Manager is implemented by the CU Manager. The Agent is deployed on the
DCR to expose its resources and execute the tasks pushed by the CU Manager. As
such, the Agent is a pilot.
...
DCR dependences; 2. the user describes also one or more pilots choosing the
...
SAGA-python API. SAGA subsumes under a unified and coherent API diverse DCR
middleware. Thanks to SAGA, RADICAL-Pilot can submit pilots to all the resources
exposed by XSEDE, by the OSG HTCondor pools, and by HPC systems like NERSC
Hopper, OLCF Titan, or NCSA Bluewaters. Adding support for new DCR is relatively
simple, as proven for example with the LRZ SuperMUC Petascale System or the
recent inclusion of a connector for public and private IaaS DCRs.
...
% 4.3
%
\subsection{Implementation Context and Auxiliary Properties}
\label{sec:context}
...
in~\S\ref{sec:understanding}. As such, these sets are minimal and, informally,
necessary to every \pilot implementation. This is not surprising as modules
implementing \pilot deployment, and task submission and execution guarantee the
very possibility of executing task-based workloads by means of multi-level
scheduling on DCRs.

The analysis of DIANE, HTCondor Glidein, GlideinWMS, PANDA, and RADICAL-Pilot
show also clear implementation differences. These are here reviewed and analyzed
on the base of the list of auxiliary properties described
in~\S\ref{sec:auxprops}.
...
in~\S\ref{sec:analysis} shapes both the design and implementation of \pilot
systems. Design-wise, adding a wrapper to the executables of a distributed or
parallel application means creating a well-defined space for their localization.
This may involve not only the specification of program switches, data locations,
or library dependences but also loading modules, downloading supporting code, or
on-the-fly compilations and execution of supporting applications.
Implementation-wise, the process of localization requires tight coupling with
the target infrastructure, its capabilities, policies, and specific
configurations.
...
\pilot system and heterogeneous target DCRs.  Thus, interoperable localization
offers the opportunity to execute heterogeneous executables via \pilot systems.

% Resource Overlay

Interoperability is a necessary preconditions to utilizing multiple pilots
across diverse DCRs. Special purpose \pilot systems tend to target a specific
type of DCR. For example, PANDA targets specifically the DCRs utilized by the
ATLAS experiment but its transition to a more general purpose system required to
diversify the types of target DCRs. This diversification imposes the development
of additional capabilities to manage the execution of tasks on multiple pilots
exposing resources with diverse characteristics.

In a multi-pilot, multi-DCR scenario, the relations among resources and task
requirements increase in number and complexity. Data not only needs to be
replicated and made available across different locations but the applications
and protocols used to move data may change across target DCRs. DCR with
specialized data capabilities may have to be prioritized depending on the task
data requirements or the same may need to be done with DCR supporting specific
types of computation as, for example, for those DCR supporting GPU-intense
executions.

In this context, \pilot systems need to support higher level abstractions as,
for example, that of resource affinity. The correlation among data, compute, and
network resources needs to be assessed for every execution and carefully
leveraged to optimize not only time to completion but also resource utilization.
These optimizations may be the determining factor to enable the execution of
...
file-centric. Data are expected to be aggregated into input/output files with
the main requirement of been transferred between filesystems across the
networking infrastructure. This assumptions may be limiting, especially when
considering data-intensive applications.

Also in this case, support for diverse technologies and higher-level
abstractions is likely to become a requirements for the future development of
general purpose \pilot systems. For example, transparent support for distributed
filesystems like HDFS~\cite{borthakur2008}, opportunistic creations of data
clusters like those utilized by YARN~\cite{vavilapalli2013}, transparent
integration of databases or storage services are likely to play a relevant role
in the efficient execution of data-oriented distributed
applications~\cite{zhang2010}.
...
This is exemplified when considering cloud-based DCIs and how their support has
been added to GlideWMS and PANDA, and RADICAL-Pilot.

A pilot agent can be executed within a VM, retrieving and executing tasks on the
resources of (one or more) VM.  As shown by the reviewed \pilot system
implementations, conceptually the role and functionalities of the \pilot agent
are the same when used in a IaaS, HPC, or grid infrastructures. The
implementation details are isolated within the agent code itself and have no
bearing on the overall design of the \pilot system.

This analysis highlights the generality of the notion of resource placeholder
and its independence from the specificities of the target infrastructures and
their middleware. Placeholders abstract the notions of resources, scheduling,
and task execution, creating a well-defined and isolated logical space for the
management of task execution.

% Interfaces

Importantly, an analogous degree of isolation is offered between the \pilot
system implementations and the application layer. The scientific application
landscape is diverse and fragmented, especially when considering distributed
applications~\cite{jha2013}. The current state of the workflow
systems~\cite{taylor2014} (a type of distributed application) is a paradigmatic
example of this situation.

Many workflow systems have been implemented with significant duplication of
effort and limited means for extensibility and interoperability. One important
contributing factor to these limitations is the lack of suitable, open, and
possibly standard-based interfaces for the resource layer. Most workflow engines
are developed with proprietary solutions to access the resource layer, solutions
that cannot be shared with other engines and that often serve specific
requirements, use cases, and infrastructures.

Open APIs like the one specified in~\cite{luckow2012b} offer the missing layer
on which workflow and, more generally, distributed applications could be built
upon. As with the resource layer, \pilot systems offers a well-defined and
...
develop special-purpose, vertical, and end-to-end applications, the main source
...
and accounting processes (AAA). The AAA requirements specific to \pilot system
...
The AAA used by the user to access her own pilots varies depending on the
...
to a pilot is part of the group of allowed users. Among the \pilot systems
reviewed, GlideinWMS is the only one implementing pilot multitenancy and using
advanced abstractions like VOs and federated certificate
...
they only need to present the credentials provided by the application layer to
the DCR. As such, the requirements for AAA implementation within \pilot systems
are minimal.
...
\item Use three dimensions (D (dependency) C (coupling) and H (heterogeneity) )
  (D I H) to analyze types of workloads. \jhanote{Is ``I'' independent?. Also we
    might want to add ``N'' -- cardinality, which captures the number of tasks
    in the workload.}
