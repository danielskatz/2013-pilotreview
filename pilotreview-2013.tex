...
in terms of target DCRs and  application workloads. Examining these
\pilot systems using the common vocabulary defined in~\S\ref{sec:termsdefs}
exposes similarities and differences allowing a detailed analysis. Critically
assessing these differences allow to bring to the fore the generality of the
pilot abstraction alongside its independence from specific software systems and
...
Components}, the \vocab{Functionalities}, and the \vocab{Terms and Definitions}
introduced in~\S\ref{sec:understanding} are highlighted in \vocab{Bold}.
\prop{Italic} is used to refer to the core \prop{Properties} just described and,
when necessary, the specific terminology used in the documentation of the \pilot
systems under analysis is distinguished by using the \impterm{Typewriter} font.
...
\mtnote{Pilot systems analyzed: DIANE, HTCONDOR, PANDA, RADICAL Pilot. Explain why so few: we now introduce 4 examplar cases showing how systems developed to execute different types of application on diverse DCRs... Different capabilities... Same core components/functionalities/properties -> Therefore: general paradigm, best practices for the implementation, choosing criteria.}
...
DIANE (DIstributed ANalysis Environment) has been developed at CERN to support
the execution of workloads on both local and EGI/WLCG Grid DCRs. DIANE has since
been used also in the Life Sciences \mtnote{Reference needed} and in few other
scientific domains \mtnote{Reference needed}.

DIANE is a application task coordination framework for those distributed
applications that can be executed by means of the \MW
pattern~\cite{Moscicki:908910}. DIANE consists of four components: a
TaskScheduler, an ApplicationManager, a SubmitterScript, and a set of
ApplicationWorkers. The TaskScheduler and the ApplicationManager are implemented
as a RunMaster service while the ApplicationWorkers as a WorkerAgent service.
...
Accordingly, the \pilot provision functionality is implemented by the
...
service represents an abstraction of the data capabilities offered by the DCR
middleware and it is designed to handle data only in the form of files stored
into a file system.
...
factories systems to support a sustained provision of pilots over extended
periods of time.
...
middleware~\cite{Moscicki20092303}. The main goal of GANGLIA is to facilitate
the submission of WorkerAgents (i.e. pilots) to diverse DCRs by means of a
uniform interface and abstraction.
...
consists of an executable invocation but also of a set of instructions, OpenMP
...
will happen. It might become 4.2.5 or a stand-alone 4.3. To be decided.}
...
\item Use three dimensions (D I H) to analyze types of workloads.
