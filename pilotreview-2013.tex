...
\jhanote{place somewhere. refine to better provide context.} HTCondor system has
evolved over the years from being a batch-queue system to supporting the latest
incarnation of distributed systems: scavenger and volunteer computing, grids and
clouds. This includes the gradual evolution to of HTCondor to support
pilots. Not surprisingly thanks to this historical evolution, the glidein pilot
capabilities are arguably more strongly coupled to the ``base'' HTCondor system
than most other pilot systems are coupled to a base system. As a consequence,
there is a need to appreciate the basic components of HTCondor.


% Who did it and for what scientific domains.
The HTCondor Glidein Service implements pilots within regular Condor pools.
Developed by [...] its main goal was originally to include DCRs exposing grid
middleware into Condor pools. The HTCondor Glidein Service has been complemented
by GlideinWMS, a Glidein-based workload management system that automates
deployment and management of glideins on multiple types of DCR middleware.
\mtnote{Find out metrics on usage and target scientific communities}
\jhanote{just a reminder that Glidein might not be service but just jobs, as
  discussed at:
  http://www.uscms.org/SoftwareComputing/Grid/WMS/glideinWMS/doc.prd/index.html}

% Architecture.
HTCondor Glidein Service has been designed as part of the service ecosystem of
HTCondor. The HTCondor components relevant to the Glidein Service are: a set of
Schedd and Startd daemons, a Collector, and a Negotiator. Schedd is a queuing
system that holds workload tasks and Startd handles the DCR resources. The
Collector and the Negotiator are implemented within a service called `Condor
Central Manager'. The Collector holds references to all the active Schedd/Startd
daemons, and the Negotiator matches tasks queued in a Schedd to resources
handled by a Startd.

GlideinWMS build upon the HTCondor Glidein Service by adding the following
components: a set of Glidein Factory daemons, a set of VO Frontend daemons, and
a VMS Collector. Glidein Factories submits jobs to the DCRs middleware, each VO
Frontend matches the tasks on one or more Schedd to the attributes of a specific
Glidein Factory, and the VMS Collector holds references to all the active
...
system as described in~\S\ref{sec:understanding}. The set of VO Frontend and
Glidein Factory, and the WMS collector implement a Pilot Manager alongside its
pilot provisioning functionality, while the set of Schedd, the collector, and
the negotiator implement a Workload Manager alongside its task dispatching
functionality. The Startd daemon implement a Task Manager alongside its task
execution functionality. A glidein is a job submitted to a DCR middleware that,
once instantiated, configures and executes a Startd daemon. As such, a glidein
is a pilot.
...
% Execution model.
The execution model of the HTCondor Glidein Service assumes that the user submit
a job (i.e. a glidein) to a DCR batch scheduler and that once executed, this job
bootstraps a working Startd daemon. This Startd daemon advertises itself with
the Collector. The user submits the workload tasks to the Schedd daemon that
advertises them with the Collector. The Negotiator matches the requirements of
the jobs to the properties of one of the available Startd daemon (i.e. a
glidein), and communicates the match to the Schedd that submits the tasks to the
indicated Startd daemon.

GlideinWMS extends the execution model of the HTCondor Glidein Service by
automating the provision of glideins. The user does not have to submit glidein
directly but only tasks to Schedd. Every Schedd advertises its tasks with the VO
Frontend that matches their requirements with the resource properties advertised
by the WMS Connector. On the base of these matchings, the VO Frontend places
requests for glidein instantiation to the VMS Collector that contacts the
appropriate Glidein Factory to execute the instantiation request. Once the
requested glideins become active on the DCRs, the advertise their availability
with the (HTCondor) Collector. From there on the execution model is the same as
described for the HTCondor Glidein Service.
