...
to every \pilotjob implementation. \mrnote{"are necessary" - "have to" sounds
...
components. \alnote{Without getting into some more analysis for Corral
  ...
  what is in \S2 or will be in \S4.}
...
Communication and coordination are two fundamental characteristics of every distributed architecture but none of their specific paradigm is distinctive of \pilotjob systems. \alnote{Is communication and coordination a component or a characteristic? (see P* paper)}\mtnote{Done.} For example, as seen in \S2.2.2, the \MW paradigm is very common among \pilotjobs. Functionally, the Master can be  identified with the architectural component concerning workload management while the Worker with the one concerning the execution of the workload's tasks. However, the \MW paradigm usually implies a specific distribution of capabilities between Master and Worker, and, in some cases, also a communication model. Both these characteristics are not necessary to implement a \pilotjob as shown, for example, by .... and .... .\mtnote{AL/all: any suggestions to fill these dots --- see note above?}\mrnote{I think there are two sides to this: not just that M-W implies a specific distribution of capabilities but that M-W can be used to apply to a non-fixed set of capabilities. For instance, M-W is used to refer to applications, frameworks, architectures, etc, as such, a clear definition of M-W does not exist. The ellipses marks can be filled in an easier manner if we change the sentence to be positive from its currently negative state. For example, as SJ says above M-W can be ``associative'' - i.e. ``while the M-W concept can be applied to Pilot-Jobs, it's ambiguity and wide-range of capabilities make it difficult to define as a necessity for a PJ system.''}
...
\item[Provisioning.] Leveraging the capabilities offered by a DCI to
  execute one or more pilots. \jhanote{I don't think we should provide
    pilot specific definitions for generally applicable terms}
\end{description}
