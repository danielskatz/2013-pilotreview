...
\alnote{move functional/non-functional after overview...}
...
execution of workloads on Distributed Computing Resources (DCIs) such as, for
example, Grids, Clouds or HPC facilities. This is achieved differently,
depending on use cases, design and implementation choices and on the
constraints imposed by the peculiarities of the targeted DCIs. Finding common
denominators among these systems require an analysis on multiple dimensions.

\pilotjobs can be analyzed and compared by looking at their architectural, deployment and workload execution characteristics. Each of these aspects involves specific properties and functionalities, some peculiar to a given \pilotjob system, others related to the \pilotjob paradigm itself. The latter are what distinguish \pilotjobs from other types of distributed systems. Isolating them means to find the common denominators among every \pilotjob system.

At an high level of abstraction, all the \pilotjob systems introduced in \S2 leverages an architecture based on two separate but intercommunicating logical components: one related to the management of a workload and its tasks, the other to the execution of the tasks on the targeted resources. As it will be explained in \S4, the implementations of these two components greatly varies from system to system with one or more elements responsible for specific functionalities both at application or infrastructure level. Nonetheless, the two architectural components can be distinguished across all the \pilotjob systems. 

For example, looking at the core \pilotjob systems introduced in \S2.2.2, the scheduler and dispatcher of Nimrod-G belong to the architectural component dedicated to workload management while the job-wrapper belong to that relative to the execution of tasks. Similar examples can be given for the advanced \pilotjob systems and for the Pilot-Based frameworks. GlideinWMS offers an extended set of capabilities when compared to the scheduler and dispatcher of Nimrod-G but all of them belong to the same, well-defined and self-contained architectural component. The same is valid for Corral or SWIFT. They both offer evolved functionalities and interfaces form managing workloads but, as such, they both are part of an architecture where workload management and task execution belongs to two distinct and intercommunicating components.

It is worth noting that the \MW paradigm can be considered a further specification of the architectural features here described. A Master can be  identified with the architectural component concerning workload management while the Worker with the one concerning the execution of the workload's tasks. However, the \MW paradigm usually implies a specific distribution of capabilities between Master and Worker, and, in some cases, also a specific communication model between the two. On the contrary, the common traits among \pilotjob architectures are agnostic about the feature amount and distribution and does not impose any specific communication model. 
...
\jhanote{I think all the functionality of PJs is predicated on the
  ...
  functionality?} \mtnote{Following our discussion I would reformulate as whether late binding is a defining capability of \pilotjob. At the moment I am not sure but I will get on it once I will have properly defined `late binding'}

The architectural properties of \pilotjob are functional to the implementation of those capabilities that allows for workloads to be executed on the targeted DCIs. Assuming that communication among components is a necessary trait for every type of distributed system, \pilotjobs included, a minimal set of capabilities needs to be implemented in order to manage workloads and execute tasks.
\begin{itemize}
  \item Assigning one or more tasks to one or more pilot jobs.
  \item Staging in and out the data required and produced by the execution of a task.
  \item Executing a task on the targeted DCI. 
\end{itemize}
...
\jhanote{I understand the spirit/intent of the above and I agree with
  ...
  that it has no loose ends :) however we should still fix.. )} \mtnote{yep, let me fix this.}

The deployment procedures of \pilotjob systems also greatly varies \[...\]


\begin{center}
\line(1,0){250}
\end{center}
\mtnote{from now on the whole section needs extensive rewriting.}
\begin{center}
\line(1,0){250}
\end{center}
...
\begin{itemize}
\item Why did we pick this specific PJ? 
\item What was this PJ designed for (motivates architecture)
\item What applications are using this PJ (if any?)
\item What is the typical deployment and usage scenario (e.g., VO. multi-user)
\item What is the resource landscape it assumes (Clouds, HPC, 'Grid', single machines,... )
\item Architecture
  \begin{itemize}
