...
with pilot-like characteristics. In~\S\ref{sec:understanding}, the hetoregeneity
...
system. In~\S\ref{sec:analysis}, the focus shifts from analysing the design of
a \pilot system to  leveraging the terminology defined
in~\S\ref{sec:understanding}
...
\begin{figure}[t]
  \centering
    ...
    \caption{Introduction of systems over time.}
    \label{fig:timeline}
\end{figure}
...
\begin{table*}
 ...
 \caption{\textbf{Mapping of the core terminology of \pilot systems into
  ...
  \S\ref{subsec:compsandfuncs}.}\msnote{What are the considerations for the mapping?}}
...
%and remains in contact with it to control its \pilotjobs.
...
% Further, an automatic re-execution of WUs is possible.
...
%% size of 1 core. 
%% To address this issue a so-called multinode submitter script can be used: the scripts starts a
%% defined number of worker agents on a certain resource.
%% However, WUs will be
...
%% propagation mechanisms are in place.
...
Pilot concept is in its heart centered around Condor Glideins. Many WMS make
use of the core functionality, ranging from many specific WMS's to the
``generic`` GlideinWMS.
Main HTCondor distribution provided command line utils to setup these Glideins.
These are now replaced by Bosco.
...
BigJob is a Python library and therefore user interface with it via the Pilot
API.
