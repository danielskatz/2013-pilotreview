...
is to offer a paradigmatic description of a \pilot system and a
well-defined vocabulary to reason about such a description and,
...
to throughput and execution time-to-completion (cit), but other metrics
could be also considered as, for example, energy efficiency, data
transfer, scale of the workload executed or a mix of them. In order to
maximize or minimize the chosen set of metrics, each \pilot system
exhibits characteristics that are both common or specific to one or more
implementations. Discerning these characteristics require isolating and
defining the minimal set of logical components that has to characterize
every \pilot system.
...
alongside information on the kernel that will be executed on the DCIs
...
a workload from the need to submit its tasks via the DCI middlware. The
...
dynamic coordinator election, local or global information sharing, or
\MW. The same applies to specific network architectures and protocols.
Different network architectures and protocols may be leveraged within a
\pilot implementation in order to achieve effective communication and
coordination.
...
referring to \pilot systems. It is not a case that \pilot systems are
...
given infrastructure. It should be noted that, given their functional
...
Once clarified the meaning of `job' and `pilot', three more terms
fundamentally associated to \pilot systems (cit, cit, cit) need to be
defined: `Mulit-level scheduling' and `late/early binding'.
...
binding is critical in assuring the aforementioned high throughput by
allowing task execution without queuing time or container instantiation
time.

\begin{description}

\item[Early binding.] Binding one or more task to an inactive pilot.

\item[Late binding.] Binding one or more task to an active pilot.
