...
This paper offers a comprehensive analysis of the \pilotjob abstraction
assessing its evolution, properties, and implementation into multiple
\pilotjob software systems.  \pilotjob systems play an important role in
supporting distributed scientific computing by providing, for example, more than
700 million CPU hours a year to the Open Science Grid communities, and by
processing up to 5 million jobs a week for the ATLAS experiment. \pilotjob
systems are also witnessing an adoption beyond traditional domains with the
increasing importance of task-level parallelism in high-performance computing.
Notwithstanding the growing impact on scientific research, there is no agreement
upon a definition of \pilotjob system and no clear understanding of the
underlying \pilot abstraction and paradigm. This lack of foundational
understanding has lead to a proliferation of unsustainable \pilotjob
implementations with no shared best practices or interoperability, ultimately
hindering a realization of the full impact of \pilotjobs.  This paper offers the
conceptual tools to promote this fundamental understanding while critically
reviewing the state of the art of \pilotjob implementations. Five main
contributions are provided: (i) an analysis of the motivations and evolution of
the \pilotjob abstraction; (ii) an outline of the minimal set of distinguishing
functionalities; (iii) the definition of a core vocabulary to reason
consistently about \pilotjobs; (iv) the description of core and auxiliary
properties of \pilotjob systems; and (v) a critical review of the current state
of the art of their implementations. These contributions are brought together to
illustrate the defining characteristics of the \pilotjob paradigm, its
generality, and the main opportunities and challenges posed by its support of
...
Falkon~\cite{raicu2007}, MyCluster~\cite{walker2006creating} to name a few.
These systems are for the most part functionally equivalent and motivated by
similar objectives; nonetheless, their implementations often serve specific use
cases, target specific resources, and lack in interoperability.
...
for third-party systems like Bosco~\cite{weitzel2012campus}.
...
Explosion'' of \pilotjobs in the first decade of the millennium, e.\,g.\
...
GWPilot pushes instead the boundaries of implementation
...
and reliable scheduling algorithms based on DCR characterization. Scheduling can
be customized at user level and the application level is well isolated from the
\pilot system level. This is an important feature because it reduced the
overhead of executing legacy code via GWPilot.
...
system functionalities from those of other middleware and an appreciation of the
...
components of a \pilotjob system may communicate or coordinate using any
suitable pattern, e.g., one-to-one, many-to-one, or
one-to-many~\ref{comm_patterns} patterns with a push or pull model, or
blackboard, meeting, or \MW patterns~\ref{omicini2013coordination,}. The same
...
administrative domain like, for example, XSEDE, OSG, CERN, NERSC, or a
University.
...
system'' will be used instead, as the term ``job'' in ``pilot-job'' identifies
...
``level'' is left undefined. It is not clear what constitutes a level or how its
boundaries should be assessed. ``Multi-entity'' and ``multi-stage'' are better
terms to describe the scheduling properties of \pilot systems, as these terms
...
boxes one inside the other on the middle-left represent a DCR. Of the four
boxes, the outmost denotes the DCR boundaries, e.g., a cluster. The second box
the container used to schedule a pilot on the DCR, e.g., a job or a virtual
machine. The third box represents the pilot once it has been instantiated on the
DCR, and the fourth box represent the resources held by the pilot.
...
the separation between scheduling at pilot level and scheduling at workload
manager level, highlighting the four entities involved: jobs on DCR middleware,
and tasks on pilots.
...
interface to the user. \jhanote{to what? for what?}\mtnote{Better?} DIANE,
...
example, in Ref.~\cite{luckow2014pilot,santcroos2012} the concept of \pilotdata
was conceived to be fundamental to dynamic data placement and scheduling as
...
support task parallel applications (and the many incarnations of
task-parallelism such as many-task computing, ensemble-based computing,
...
concurrent execution of multiple tasks that comprise a single application,
arises due to the job-centric resource allocation and management viewpoint still
pervasive high-performance machines. As alluded to, \pilots via placeholders
provide {\it application-level} control of the resources, where {\it
  application-level} is a proxy for raising the level at which control of
resources resides. This could be at the end-user application, or at an
intermediary level, e.g., \pilots can serve as important run-time systems to
support non-traditional and heterogeneous workloads on supercomputers.  In fact,
the ability to provide a run-time system to support many short running tasks on
Blue Gene computers was a motivation for the development of
Falkon~\cite{raicu2007}.
...
implementations and functionality that a set of \pilot ``building blocks'' that
help find the balance between specific and general should be possible.  It is
also possible to have well-defined models of \pilot functionality, as well as an
unambiguous description of properties. Collectively these should be reflected in
external/internal interfaces that both developers and users can rely upon, which
would help address the aforementioned issue of number of active \pilot systems.
Furthermore the commonality amongst \pilot systems is arguably unique amongst
other tools and middleware, which thus allows \pilot systems to serve as an
interesting case-study in to the underlying reasons for the proliferation of
functionally similar but otherwise distinct software systems.  An analysis of
software decisions and development trajectory of the \pilot implementations
could provide insight into how sustainable software ecosystems might evolve.
...
arbitrarily specification. Thus the type of matching between the requirements of
tasks and the capabilities offered by the resources held by the pilot should not
be mandated. For this reason, adopting the \pilot paradigm for the execution of
...
improving their interoperability across specific DCR and curtailing the need to
create unsustainable partial implementations.
