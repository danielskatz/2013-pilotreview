...
HT Condor. The components relevant to the Glidein Service are: a set of Schedd
and Startd daemons, a Collector, and a Negotiator. Schedd is a queuing system
that holds workload tasks and Startd handles the DCR resources. The Collector
and the Negotiator are implemented within a service called `Condor Central
Manager'. The Collector holds references to all the active Schedd/Startd
daemons, and the Negotiator matches tasks queued in a Schedd to resources
handled by a Startd.

GlideinWMS build upon the HTCondor Glidein Service adding the following
