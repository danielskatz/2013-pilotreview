...
\mtnote{\MW? ;) I see that later on you use `Workload manager' and
  `task executor'. Would you be happy with them? Would the
  others?}\\---\\
...
\onote{job-wrapper seems to be an arbitrary term?}\mtnote{Is the the
  term used in Section 2. Before using it I checked and found it used
  in this Nimrod-G paper
  (http://arxiv.org/ftp/cs/papers/0009/0009021.pdf)}

\onote{Flow interrupted w.r.t., prev. paragraph}\mtnote{I am not sure
  I got the comment right but, if so, I added a `cap' to the beginning
  of the following paragraph to avoid a sudden change of
  topic.}\\---\\
