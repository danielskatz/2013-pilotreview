...
The goal of this Section is twofold. \jhanote{More than twofold? I
  think we are missing the contributions of section 4.1 and 4.2}
Initially, within the boundaries set by the minimal description
offered in \S\ref{sec:vocab}, it is shown how each \pilotjob system
provides specific sets of features. Such features are then classified
and clustered across \pilotjob systems. In this way, indications are
offered about how to choose a pilot system based on functional
requirements, how \pilotjob systems are designed and engineered, and
the theoretical properties that underly such systems.



\jhanote{Matteo: I think the order of 4 differs from the message
  conveyed in the previous paragraph. Also, I think the clustering is
  the reverse of what is suggested, viz., the properties are
  defined/derived, and then pilot-jobs systems are clustered around
  the properties and not vice-versa. agreed?}
...
implementations. Two sets of properties are introduced: Core and
auxiliary.  \textit{Core} properties are common to each \pilotjob
implementation while the auxiliaries characterize specific \pilotjob
implementations and they are not shared among all implementations.
More importantly however, auxiliary properties are not unique to
\pilotjob and are properties that can be found in other tools and
services. \jhanote{Give reader a reminder that ``the set'' of core
  properties are both necessary and sufficient. Also, maybe this is
  intended or implied, but it will help to make explicit that
  auxiliary properties are not a closed set of properties, unlike
  core}
  

Core and auxiliary properties are chosen in accordance with the set of
functionalities described in \S\ref{sec:vocab} (Table
\ref{table:core_properties}). Each \pilotjob system is reviewed so as
to outline the differences among the implementations of Pilot
Provisioning, Task Dispatching, and Task Execution. Pilot provisioning
is reviewed \jhanote{analyzed?} by focusing on the requirements and
dependencies between the \pilotjob system and the underlying
distributed resources, and the type of resources the pilot system
exposes in terms of computing, data and networking.  \jhanote{please
  elaborate on the previous sentence, maybe with an example?} For Task
Dispatching, two specific core properties \jhanote{``core'' should
  probably not be used} are considered: The semantics of workloads,
and how the tasks of a given workload can be bound to single or
multiple pilots. Finally, Task Execution is analyzed by reviewing
\jhanote{inconsistency: are we analyzing or are we reviewing. please
  fix} different strategies for task scheduling and by describing how
\pilotjob systems support specific execution strategies for multiple
types of workload.
...
\begin{table*}[t]
 ...
 \begin{tabular}{|l|l|}
  \hline
    \textbf{Core Propriety} & \textbf{Functionality} \\
  \hline
    \textbf{Resource Dependences} & Pilot Provisioning \\
  \hline
    \textbf{Pilot Types} & Pilot Provisioning \\
  \hline
    \textbf{Workload Semantics} & Task Dispatching \\
  \hline
    \textbf{Binding Types} & Task Dispatching \\
  \hline
    \textbf{Instantiation Strategies} & Task Execution \\
  \hline
    \textbf{Execution Strategies} & Task Execution \\
 \end{tabular}
 \caption{\textbf{\mtnote{First draft with the revised naming for the Core 
            Properties.}Mapping of the Core Properties of \pilotjob system 
...
\pilotjob systems implementations without recourse to the core
properties of Pilot Provisioning, Task Dispatching, or Task
Execution. Interface, middleware and usage modes supported,
multi-tenancy, data management and transfer, authentication,
authorization, and interoperability are all examples of properties
that characterize each component of a \pilotjob implementation. As
such, auxiliary properties are considered global properties of the
\pilotjob system without defining any of its distinguishing
features.\mtnote{I am rambling here, maybe something more insightful
  will come to mind after leaving this alone for few hours. Mark, any
  insight on why and how Auxiliary properties are relevant to our
  critical review of PJ implementations?}  \jhanote{``global'' may
  need some qualification, as I can't discern what is being suggested}
...
support workflow-based applications, others jobs leverage a Message
...
execution strategies that maximize throughput (HTC), computing (HPC) or
...
\mtnote{TODO: . Change the naming of the Core Properties once we will have
agreed about the naming used in Table 2. . Define each property not in
interrogative form.}
...
\item \textit{Binding behavior:} Does the \pilotjob system allow to specify
\textbf{early} and/or textbf{late binding}?

% \mtnote{I am not sure we should be adopt and application-centric vision of the
% core proprieties of \pilotjob implementations. This paper is about \pilotjob
% systems, not about how application use/interact/bound with them. So I think
% here we understand whether we care about an application been able to do specify
% early/late binding or for a \pilotjob implementation to allow for this
% distinction among type of bindings to be made at all.} \msnote{I think your
% suggestion for scoping makes sense. I would be happy to "implement" that.}

\item \textit{Resource exposure:} What knowledge (if any) does the pilot layer
need about the actual \textbf{resources}/\textbf{infrastructrure}? 
% \mtnote{See the comment above about an application-centric approach.}

\item \textit{Pilot characteristics:} Whats the "nature" of the
\textbf{\pilot}? Related to this, what kind of \textbf{jobs} can
pilots be? How long do they "live"? Who/what instantiates them?

\item \textit{Pilot instantiation:} What are the scheduling strategies used to
instantiate \textbf{pilots}?

\end{itemize}

\subsubsection{Auxiliary properties}

\mtnote{TODO: . Change the naming/list of the Auxiliary Properties after
finalizing the paragraph that introduce them at the beginning of the
subsection.}
...
\begin{itemize}
  ...
  \item \textit{Pilot ownership}: Who owns the pilot (job) -- the user or the \pilotjob system? Or in other
    words, in which security context does the pilot operate?  \msnote{Dont
    think we have defined "user" by now, so thats ambiguous} \msnote{As there
    will be a wide spectrum of approaches in the various implementation, we
    might just want to call this as general as "security"}
    \aznote{I think this covers potentially multiple concepts: 1) security, as
      ...
      force a kill directly?  Modify pilot configurations while it runs?)
      2) Deployment, as in does the entire pilot-job system operate
      in user-space or is it part of the (admin/privileged) system stack?
      3) I would argue that this could relate directly to pilot-instantiation...
      both from a security and deployment level.  Who can instantiate pilots?
      What security level do they run on once instantiated?  So, maybe a ``security''
      item as Mark suggested, but I'm not entirely sure if ``deployment'' should
      be part of this as well...  they do seem connected to me though...}
...
\begin{itemize}
  ...
  \item \textit{Supported ``logical'' workload types}: What types of workload are supported, e.g. 
    ...
    \msnote{It seems this is a special case of the above item}
...
\begin{itemize}
  ...
  \item \textit{Task-pilot relationship (temporal)}: What is the lifetime of a pilot in relation to the 
    length of a task or workload? \msnote{What kind of answers to you expect
    here in section 4.3?} 
