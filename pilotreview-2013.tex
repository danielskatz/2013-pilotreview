...
As described in Tables~\ref{table:core_properties}, Pilot Resources and Pilot
Deployment represent the types and characteristics of the resources held by the
pilot, and the modalities for pilot scheduling, bootstrapping, and aggregation.
Both properties characterize the implementation of a Pilot Manager and its Pilot
Provisioning functionality. Workload Semantics describes the properties and
relations among tasks captured in a workload description while Workload Binding
the modalities and policies for binding tasks to pilots. These two properties
identify the implementation of a Workload Manager and its Task Dispatching
functionality. Workload Execution identifies the type and characteristics of the
environment in which tasks are executed. As such, this property characterizes
the implementation of a Task Manager and its Task Execution functionality.
...
Pilot Deployment &
Modalities for pilot scheduling, bootstrapping, and aggregation &
Pilot Manager &
Pilot Provisioning \\
...
\item \textbf{Pilot Deployment}. Modalities for pilot scheduling, bootstrapping,
  and aggregation. The characteristic of these operations varies depending on
  the design choices of the \pilot systems. For example, pilot scheduling may be
  fully automated (i.e. implicit) or directly controlled by applications and
  end-users (i.e. explicit). Pilots can be bootstrapped from code downloaded at
  every instantiation or from code that is bundled by the DCR. Once
  bootstrapped, pilots can be independently used or aggregated into a resource
  overlay presented as an homogeneous set of resources to the application layer.
  Scheduling and bootstrapping are performed on a DCR and their design depends
  on whether single or multiple types of DCRs are targeted. For example, a
  design based on connectors will be desirable when targeting multiple DCRs in
  order to gather and use information about type of container (job, virtual
  machine), type of scheduler (PBS, HTCondor, Globus), amount of cores,
  walltime, or available filesystems.
