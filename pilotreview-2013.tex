...
\caption{\textbf{Mapping of the Properties of \pilot system implementations
  ...
  \mtnote{The mapping between core properties, components, and functionalities
    ...
    columns, and reference/place them accordingly.}\jhanote{Yes, this makes sense.}
}
...
\jhanote{the last sentence is a very important sentence. It could do with
  ...
  causing confusion. Would be glad to discuss}
...
\item \textbf{Workload Semantics}. The tasks of a workload are dispatched to
  ...
  % \jhanote{The list of dependencies was not complete, hence the conversion from
  %   specifically to for example. also i removed ``affinity'' between data and
  %   compute resources, as it would be unclear to the reader.} \mtnote{I removed
  %   `(standard)' as it seemed unnecessary but please add it back if you think
  %   differently.}
...
% \jhanote{alternate style suggestion: moved the earlier last sub-sentence to the
%   beginning. i've commented out the original text in case you'd like to revert}
...
%   into a 4.1.1 component and a 4.1.2 component?} \mtnote{Went for option
%   (i). Better?}
...
\item \textbf{Communication and Coordination}. Communication and coordination
  are features of every distributed system, but \pilot systems are not defined by
  any specific communication and coordination pattern or protocol. The details
  of communication and coordination among the \pilot system components are
  implementation properties. \jhanote{I would not use the word
    distinguishing in the last sentence. OK?} \mtnote{Done.}
...
abstractions are offered. \jhanote{``bind to'' compute resources in the first
  ...
  specified as one entity that is being bound.}
...
\jhanote{I'm not sure the meaning of the previous sentence is clear: does it
  ...
  literature right, it was about getting HTCondor to work with grid
  middleware. Any specific reason because using `grid middleware' is
  problematic?}\jhanote{the objective remains to aggregate DCRs. grid middleware
  is a layer/mechanism.}
...
\mtnote{That uses as little resources as possible to execute the given
  ...
  and emphasising what it controls actively?} \mtnote{OK. I am happy with the current version.}
...
\jhanote{reference to DAGMan needed}\mtnote{Done.}
...
applications. \jhanote{Matteo: OK?}
