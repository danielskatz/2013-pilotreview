...
It should be noted that resource placeholders have the potential to  to
mitigate the side-effects introduced by multi-tenancy and to facilitate
execution across multiple supercomputers. Multi-tenancy affects only the
scheduling of the agent as it needs to be executed by the batch system of the
remote supercomputer. Once the agent is executed, the user -- or the master
process of the distributed application -- may hold total control of the
...
enabling the application to acquire resources and efficiently schedule tasks
onto these. Besides M-W, AppLeS provides also different application templates,
e.\,g.\ for parameter sweep and moldable parallel
applications~\cite{Berman:2003:ACG:766629.766632}.\mtnote{To be aggregated in
the new version}
...
master-worker framework grew \cite{Anderson:2004:BSP:1032646.1033223}.%Applications can be built on top of BOINC for their own scientific endeavors.
...
% available job slots with the central Condor pool.
