...
\item How does the \textbf{application} (responsible for definition of the
\textbf{workload} and consumer of the pilot-job system) interact/interface with
the pilot-job system? (API / CLI / Service / etc)

\item What do \textbf{tasks} constitute of? What kind/type of \textbf{tasks}?

\item How is the \textbf{workload} that is passed from the \textbf{application}
to the pilot-job system described? E.g.: What is the syntax? What are the
semantics? (This implicitly deals with \textbf{tasks} also).

\item Does the pilot-job system allow the application to specify \textbf{early}
and/or \textbf{late binding}?

\item What knowledge (if any) does the application need about the actual
\textbf{resources}/\textbf{infrastructrure} where to run?

\item Whats the "nature" of the \textbf{\pilot} (placeholder)? Related to this,
what kind of \textbf{jobs} can pilots be? How long do they "live"? Who/what
instantiates them?

\item Security? Does the pilot job system allows \textbf{\pilots} to be used by
multiple users?

\end{itemize}

Classifiers/properties/mystical terms/etc:
\begin{itemize}

\item \textbf{Interface:} CLI / (Web Service) / Library / Framework

\item \textbf{Workload semantics:} BoT / DAG

\item \textbf{Binding:} Early / Late / Both

\item \textbf{Resources exposed:} Yes / No

\item \textbf{Pilot instantiation:} Explicit / Implicit

\item \textbf{Multi-user Pilot:} Yes / No

\end{itemize}


Suggested structure for "S4.3":
\begin{itemize}
