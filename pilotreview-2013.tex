...
services. \jhanote{Give reader a reminder that ``the set'' of core
  properties are both necessary and sufficient}
  

Core and auxiliary properties are chosen in accordance with the set of
functionalities described in \S\ref{sec:vocab} (Table
\ref{table:core_properties}). Each \pilotjob system is reviewed so to outline
the differences among the implementations of Pilot Provisioning, Task
Dispatching, and Task Execution. Pilot provisioning is reviewed
\jhanote{analyzed?} by focusing on the requirements and dependences between the
\pilotjob system and the underlying distributed resources, and on what type of
resources the pilot system exposes in terms of computing, data and networking.
\jhanote{please   elaborate on the previous sentence} For Task Dispatching, two
specific core properties \jhanote{``core'' should probably not be used} are
considered: The semantics of multiple types of workload and how the tasks of a
given workload can be bound to single or multiple pilots. Finally, task
execution is analyzed by reviewing different strategies for task scheduling and
by describing how \pilotjob systems support specific execution strategies for
...
\pilotjob systems implementations without being specific to Pilot Provisioning,
Task Dispatching, or Task Execution. Multi-tenancy, authentication,
authorization, and interoperability are all examples of properties that
characterize each component of a \pilotjob implementation. As such, auxiliary
properties are considered global properties of the \pilotjob system without
defining any of its distinguishing features.\mtnote{I am rambling here, maybe
something more insightful will come to mind after leaving this alone for few
hours. Mark, any insight on why and how Auxiliary properties are relevant to
our critical review of PJ implementations?} \jhanote{``global'' may need some
qualification}
