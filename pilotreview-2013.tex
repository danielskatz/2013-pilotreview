...
\mtnote{AL/all: any suggestions to fill these dots --- see note
  above?}\mrnote{I think there are two sides to this: not just that
  M-W implies a specific distribution of capabilities but that M-W can
  be used to apply to a non-fixed set of capabilities. For instance,
  M-W is used to refer to applications, frameworks, architectures,
  etc, as such, a clear definition of M-W does not exist. The ellipses
  marks can be filled in an easier manner if we change the sentence to
  be positive from its currently negative state. For example, as SJ
  says above M-W can be ``associative'' - i.e. ``while the M-W concept
  can be applied to Pilot-Jobs, it's ambiguity and wide-range of
  capabilities make it difficult to define as a necessity for a PJ
  system.''}\mtnote{I have incorporated your comment in the paragraph
...
\onote{job-wrapper seems to be an arbitrary term?}\mtnote{Is the the
  ...
  in this Nimrod-G paper  (http://arxiv.org/ftp/cs/papers/0009/0009021.pdf)}
