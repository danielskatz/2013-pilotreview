...
implement pilot deployment, a specific authentication and authorization protocol
may be required to deploy pilots by means of a particular DCR middleware. As
such, auxiliary properties are not necessarily shared among all \pilot systems.
...
minimal and complete while that of auxiliary properties is an exemplar subset.
While a minimal set of properties has to characterize a \pilot system in order
for it to implement the components and functionalities described
in~\S\ref{sec:compsandfuncs}, an arbitrary number of auxiliary properties
support can characterize \pilot systems, depending on use case and target DCRs
requirements. Note that both set of properties refer to
\pilot systems and not to individual pilot instances deployed on DCRs.
...
The core properties that have to characterize every \pilot system are derived by
analyzing what characteristics each component described in
in~\S\ref{sec:compsandfuncs} has to have in order to implement its defining
functionality.

The Pilot Manager implementation has to have at least the following
characteristics in order to provide its Pilot Provisioning functionality: pilots
need to (i) hold at least one type of resource otherwise a pilot would not be
usable to execute tasks; (ii) have a wrapper of the type required by the target
DCR otherwise it would not be possible to schedule that pilot on the chosen DCR;
(iii) have the appropriate bootstrapping environment and routines otherwise the
pilot would not become available for task scheduling and execution. These
characteristics are here grouped into two core properties: Pilot Resources and
Pilot Deployment.

The Workload Manager implementation needs at least the following characteristics
to provide its Task Dispatching functionality: (i) workloads descriptions have
to express the computing requirements of each task alongside its data and
inter-task dependences (if any) in order for the workload to be scheduled and
then executed; (ii) each pilot has to have a list of tasks and given task has to
be in the list of at least one pilot otherwise complete binding between task and
pilot(s) will not be achieved; (iii) each task list has to belong to a pilot
description to obtain early binding or to a pilot instance to obtain late
binding. Workload Semantics and Workload Binding are the two core properties
used to represent these characteristics of the Workload Manager.

The Task Manager implementation requires information about those configuration
and environmental features that determine the execution process of a task on the
chosen DCR. Without such information, Task Execution functionality could not be
implemented as the execution of a task would fail for misconfiguration or, for
example, for lack of required libraries, modules, or coordination with
supporting execution software. This requirements are here indicated with the
core property named `Workload Execution'.
...
Pilot Resources &
Types and characteristics of the resources held by the pilot &
Pilot Manager &
Pilot Provisioning \\
...
Workload Semantics &
Properties and relations among tasks captured in a workload
description &
Workload Manager &
Task Dispatching \\
...
Workload Execution &
Type and characteristics of the environment in which tasks are executed &
Task Manager &
Task Execution \\
...
\item \textbf{Pilot Deployment}. Modalities for pilot scheduling and
  ...
  automated (i.e. implicit) or directly controlled by applications and end-users
  (i.e. explicit). Pilots can be bootstrapped from code downloaded at every
  instantiation or from code that is bundled by the DCR. Scheduling and
...
\item \textbf{Task Binding}. Modalities and policies for binding tasks to
  ...
  \pilot systems can support application-level or multi-level scheduling decisions. For example, coupled tasks may have to be bound to
  a single pilot, loosely coupled or uncoupled tasks to multiple pilots; tasks
  may be scheduled to a pilot and then to a specific pool of resources on a
  single compute node; or task binding may be prioritized depending on task size
  and duration.

% core because: its the only property of the task executor
% component/functionality
\item \textbf{Task Execution}. Type and characteristics of the
  environment in which tasks are executed. Once bound to a pilot, a task needs
  an environment that satisfies its execution requirements. The execution
  environment depends on the type of task (e.g., single or multi-threaded, MPI),
  task code dependences (e.g., compilers, libraries, interpreters, or modules ),
  and task communication, coordination and data requirements (e.g.,
  interprocess, internode communication, data staging, sharing, and
  replication).
