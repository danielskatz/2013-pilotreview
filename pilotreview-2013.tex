...
\item \textbf{Pilot Management}. The total set of resources fostered by
  ...
  system is a major implication.\mtnote{We may want to expand this to
  scheduling and aggregation of pilots. This would overlap with Infrastructure
  Interaction though. Maybe eliminate Infrastructure Interaction absorbing it
  into Pilot Management? Should we argue that Infrastructure Interaction (e.g.
  SAGA) can also be used to coordinate Task Binging/Execution?}
...
community.
Is often cited as the project that introduced the \pilot concept\cite{}.
Similar in many respects to other batch queuing systems, HTCondor puts special
emphasis on high-throughput computing (HTC) and opportunistic computing.
...
The former can be can be achieved by applying the latter.
...
Kernel}: \impterm{Agent} (also referred to as \impterm{schedd}),
\impterm{Resource} (also referred to as \impterm{startd}), \impterm{Shadow},
\impterm{Sandbox} (also referred to as \impterm{starter}) and
\impterm{Matchmaker} (also referred to as \impterm{central manager}).

The user submits \vocab{Tasks} (in HTCondor called \impterm{Jobs}) to the
\impterm{Agent}.
\impterm{Agents} provide persistent storage for the user's \impterm{Jobs} and
tries to find computing \vocab{resources} \impterm{Resource} to execute them.

Both \impterm{Agents} and \impterm{Resources} advertise themselves to the
\impterm{Matchmaker}, which is then responsible for bringing potential pairs
together. The \impterm{Matchmaker} will notify the \impterm{Agent} of these
potential matches, and the \impterm{Agent} is on its turn responsible to
contact the \impterm{Resource} and validate the suggested match.
...
\impterm{Job} happen.
The \impterm{Agent} will start a \impterm{Shadow} which is responsible for
providing all the details required to execute a \impterm{Job} (the
executable, the arguments, the environment, the input files).

The \impterm{Resource} creates a \impterm{Sandbox} which is a appropiate
execution environment (the \impterm{Sand} for the \vocab{Task} while at the
same time protecting the \impterm{Resource} from a mishaving \impterm{Job} (the
\impterm{Box}).

Following on from this, an \impterm{Agent}, \impterm{Matchmaker} and a (set of)
\impterm{Resources} together form a \impterm{Pool}. 
\vocab{Resources} in a pool can span a wide spectrum of systems.
While some \impterm{Pools} are comprised of regular desktop PCs (sometimes
called a campus grid), other \impterm{Pools} incorporate large HPC clusters and
...
that (which was also only natural) many indepedent \impterm{Pools} were created
.
As a user (or \impterm{Agent} could only belong to one \impterm{Matchmaker},
the \impterm{Resources} that a user could access were therefore limited to one
\impterm{Pool}.
This lead to the concept of \impterm{gateway
flocking}~\cite{Epema:1996:flocking}.
With this, an individual \impterm{Pool} would be interfaced to another
\impterm{Pool} by letting associated \impterm{gateway} machines expose
information about the \impterm{Pool's} partipants to another \impterm{Pool},
e.g. about idle \impterm{Resources} or \impterm{Agents}.
These \impterm{gateway} nodes can be configured to adhere to policies and are
...
\impterm{organisations}/\impterm{communities}, and a user (or \impterm{Agent})
...
While both forms of \impterm{flocking} had their raison d'\^{e}tre, and they
could even be mixed, \impterm{gateway} flocking did not stand the test of time
...
Condor-G allows a user (an \impterm{Agent} to incorporate those HPC resources
...
\impterm{Agent} has to direct the \impterm{Job} to a specific GRAM endpoint
...
This implied that in this mode of operation there was no \impterm{Matchmaker}
...
%
To undo this coupling again, all ingredients were already available to
construct a personal Condor \impterm{Pool} out of these GRAM enabled resources.
In a three-step concept coined \impterm{Gliding In}, i) a Condor-G agents uses
...
queue connect to the \impterm{Matchmaker} started by the \impterm{Agent} and
thereby form a personal Condor \impterm{Pool}, and, iii) \impterm{Jobs}
submitted by the user to the \impterm{Agent} which are then matched to and
executed on the \impterm{Resources} within the \impterm{Pool}.

%\paragraph{Pilot Resource Capabilities}

\paragraph{Resource Interaction}

In its native mode as a batch queuing system, HTCondor is the middleware, and
therefore the discussion of resource interaction does not apply.
With the advent of Condor-G, Condor can be deployed to any site that offers a
GRAM interface.
...
This allows the \impterm{agent} to defer placement decisions until the last
possible moment.
If the \impterm{Agent} submits requests for resources to several matchmakers,
...
\paragraph{Task Execution Modes}
A \impterm{universe} in HTCondor defines an execution environment.
There are distinct implementations of these universes available: the
\impterm{Standard Universe} provides checkpointing (relevant because of the
opportunistic mode of Condor) and remote system calls, which requires
special compilation of the application though, the \impterm{Java
Universe} hides the details of setting up the remote JVM, the \impterm{Vanilla
Universe} for applications that can't be modified, the \impterm{Grid Universe}
is the mode for Condor-G, the \impterm{VM Universe} for exploiting VMware and
XEN virtual machines, and the \impterm{Parallel Universe} for executing MPI
applications.
It should be noted that these \impterm{Universes} don't mix, and that, e.g.
there is no (real) support for MPI jobs in the Condor-G mode.

\paragraph{Coordination and Communication}

The various kernel elements can be deployed in multiple configurations and can
all be run on a single machine but also fully distributed.
The HTCondor system has their history in instituational environments and assume
not too many network restrictions. The communication between the kernel
components consist of both UDP and TCP based proprietary protocols that uses an
array of dynamically allocated ports.
With Condor-G the requirements were significantly reduced and the main
communication is with the GRAM service.
...
an appriopriate security protocol.
Regarding the execution of tasks, there is a level of trust assumed between
resource and application.

\paragraph{Files and Data}

The handling of files and data was initially not a primary design principle and
did not extend well beyond facilities to stage relatively small data into the
sandbox. The \impterm{Standard Universe} offers the ability to re-route file
I/O back to the \impterm{Shadow} of the \impterm{Agent}.
Over the years, HTCondor has been integrated with various data tools.
Out of the same stable as HTCondor comes NeST\cite{}. NeST is a software-only
storage appliance that supports a miriad of storage protocols and integrates
with the \impterm{Matchmaker} to announce its availability.
With storage systems like NeST being available, one can then use a system like
Stork\cite{} to manage the transfers to and from this storage resources.
In concordence with the \impterm{Matchmaker}, Stork can make educated decisions
about data placement.
Finally, Parrot\cite{} is a mechanism that offers a POSIX I/O interface to
applications to files that are otherwise only available remotely.
