...
of application that is executed. They can be engineered so as to support
different types of applications and application objectives. \pilots hold
resources and the properties of such resources can also be arbitrarily
specified. Thus the \pilot paradigm
...
% enabling the application to deploy their
...
multi-stage scheduling: Applications need to initialize their so-called
...
then can execute tasks in these containers. Furthermore, frameworks built upon
the \pilot paradigm are emerging for YARN. % For example, Llama~\cite{llama_url}
% offers a long-running application master for YARN designed for the Impala SQL
% engine. 
...
Independent of the Hadoop developments, Google's Kubernetes~\cite{XXX} is
emerging as an important container management approach, which not completely
