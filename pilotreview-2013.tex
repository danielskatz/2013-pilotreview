...
\begin{table*}
 ...
 \caption{\textbf{Mapping of the core terminology of \pilot systems into
  ...
  \S\ref{subsec:3.1}.}\msnote{What are the considerations for the mapping?}}
...
\msnote{There is another (sub)level arguably, where the 'agent' makes
scheduling decisions about task placement within the pilot's resource pool.
Although this is an implementation detail, it might be worth mentioning?}
...
any) argument for early binding on pilots.}
...
\multirow{5}{*}{\textit{Core}}
  & Pilot Characteristics & Pilot Manager & What pilots to start \\
  \cline{2-4}
  & Resource Dependencies & Pilot Manager & How to start pilots \\
  \cline{2-4}
  & Workload Semantics & Workload Manager & How to dispatch tasks \\
  \cline{2-4}
  & Task Binding Characteristics & Workload Manager & When to dispatch tasks \\
  \cline{2-4}
  & Deployment Strategies & Task Executor & How to execute tasks \\
\hline
...
the \pilot paradigm - we do not want to give the impression that \pilots
...
S2 when \pilot systems are introduced.}
...
not-\pilot systems; 2. Extend the `model' so to include a logical element
...
\item \textbf{Pilot Resource Capabilities}: In~\S\ref{sec:3} pilots
...
\item \textbf{Workload Semantics}: The tasks of a workload are
  ...
  capabilities they require in order to be executed.  \pilot
...
\item \textbf{Task Binding Characteristics}: One of the core
  ...
  as such, the whole \pilot system would not be usable. As seen in
  \S\ref{sec:3}, \pilot systems may allow for two main types of
  binding between tasks and pilots: early binding and late
  binding. \pilot system implementations differ in whether and how they
...
% (iv) how is the workload managed and scheduled (supported
...
% (iii) the % deployment model: hosted service versus tool/library, application vs.
% system-level,
...
\item \textbf{Interfaces}: The implementations of \pilot systems
  may present several types of interfaces. For example, there are
  interface considerations between the main components of the
  \pilot system, between the application and pilot layer, between
  end users and the \pilot system, and for one or more programming
  languages.
...
\ref{sec:3}, \pilot systems support many different use cases involving
...
\item \textbf{Data management:} Does the framework provide data management
