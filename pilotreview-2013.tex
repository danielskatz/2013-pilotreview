...
Table~\ref{table:terminology}). The Overlay Manager handles the
...
at the systems introduced in \S\ref{ssec:history}, \ldots \mtnote{I will
...
type of DCIs. As such, these functionalities are here considered
critical implementation details, depending on the specific
characteristics of the given user, workload, and targeted resources.
...
workload. Thanks to resource overlays, \pilotjob systems can also
...
container as, for example, one implemented by means of a Virtual Machine
(VM) on so-called Infrastructures as a Service (IaaS).
