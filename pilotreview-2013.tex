...
\amnote{that gives fast startup of tasks, but not high throughput.  Also, the
pilot must start up, late binding won't change that.  I know we can pick the
first pilot which becomes active -- but that is not what is being said here...}

\mtnote{I think this does not refer to backfilling but to scheduling tasks to an
active pilot. There is no queuing time for the tasks and tasks are executed as
soon as they are scheduled into the pilot (barring resource exhaustion of the
pilot itself) i.e. high throughput.}

\amnote{All else being equal, isn't the difference then that the pilot is
submitted earlier rather then the effect of early vs. late binding? Assume a
pilot is submitted a $T_0$, tasks are submitted at $T_1$, the pilot becomes
active at $T_2 > T_1$.  On both early an late binding, the tasks can only start
execution after $T_2$ -- what shifts is time of scheduling, not time of task
execution.  For a single pilot, it should not make a difference.}
...
\amnote{I absolutely agree with the definition (early binding: scheduling tasks
    to a not-yet-active pilot; late binding: schedule tasks to an active pilot).
    What puzzles me is the motivation used for late binding (high throughput of
    tasks).  I may have a different definition of 'throughput' in mind though:
    for me this refers to tasks/second.  Both a pilot used with early and a
    pilot used with late binding can be very efficient, and achieve the same
    throughput of tasks.  In fact, for a single pilot (and an ideal pilot system
    implementation), there should be no performance difference in early vs. late
    binding.  It is only in the multi-pilot case where it makes a difference --
    but not in throughput, but in total TTC.  I thought this is also what your
    experiments showed?  If that is something your data disagree with, then
    please disregard, and feel free to remove that discussion as 'solved' :)}

\mtnote{Thank you for clarifying Andre. It is indeed about distinguishing
between throughput of the pilot and that of the distributed application. Here we
refer to the latter. About the one of the former, we are in agreement. I
modified the paragraph accordingly. Any better?}
...
\item \textbf{Workload Semantics}. The tasks of a workload are dispatched to
  ...
  \mtnote{I agree. Indeed the kernel of the task is not mentioned among the semantic properties taken into consideration for the scheduling decision. Pilot systems that are not capable of ``dispatching decisions
  ...
  defined in 3, so in case of affirmative answer I may have to add that too. Not a problem if you consider this necessary.}
...
\item \textbf{Multitenancy}. \pilot systems may offer multitenancy at both
  ...
  think `deployment' is now covered by the homonymous core property and `service
  model' is part of the `architecture' auxiliary property.}
