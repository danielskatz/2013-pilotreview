...
\ref{table:core_properties}). Each \pilotjob system is reviewed so to
outline the differences among the implementations of Pilot
...
dependences between the \pilotjob system and the underlying
distributed resources, and on what type of resources the pilot system
exposes in terms of computing, data and networking. \jhanote{please
  elaborate on the previous sentence} For Task Dispatching, two
specific core properties \jhanote{``core'' should probably not be
  used} are considered: The semantics of multiple types of workload and
how the tasks of a given workload can be bound to single or multiple
pilots. Finally, task execution is analyzed by reviewing different
strategies for task scheduling and by describing how \pilotjob systems
support specific execution strategies for multiple types of workload.

Several auxiliary properties play a fundamental role in distinguishing
among \pilotjob systems implementations without being specific to
Pilot Provisioning, Task Dispatching, or Task
Execution. Multi-tenancy, authentication, authorization, and
interoperability are all examples of properties that characterize each
and every component of a \pilotjob implementation. As such, auxiliary
properties are considered global properties of a system without
defining any of its distinguishing feature. \mtnote{I am rambling
  here, maybe something more insightful will come to mind after
  leaving this alone for few hours. Mark, any insight on why and how
  Auxiliary properties are relevant to our critical review of PJ
  implementations?} \jhanote{``global'' may need some qualification}
...
engineered and deployed. For example, while every \pilotjob system
offers the opportunity to schedule tasks of a workload on a pilot,
the degree of support for specific workloads vastly varies across
implementations. Some \pilotjob systems support workflows- based
applications, others jobs that leverage a Message Passing Interface
(MPI). Analogously, every \pilotjob systems support the execution of
one or more type of workload but they differ when considering
execution strategies that maximize throughput (HTC), computing (HPC)
or container-based, high scalability (Cloud).
...
\begin{table*}[t]
 ...
 \begin{tabular}{|l|l|}
  ...
  \hline
    \textbf{Workload/Task Semantics} & Task Dispatching (and Task Execution 
    in the future version of S3) \\
  \hline
    \textbf{Binding Behavior} & Task Dispatching (and Task Execution 
    in the future version of S3) \\
  \hline
    \textbf{Resource Exposure} & Pilot Provisioning \\
  \hline
    \textbf{Placeholder Characteristics} & Pilot Provisioning \\
  \hline
    \textbf{Pilot Instantiation} & Pilot Provisioning \\
  \hline
 \end{tabular}
 \caption{\textbf{\mtnote{Update table. At the moment it is OUTDATED.}Mapping of the Core Properties of \pilotjob system 
...
application to specify \textbf{early} and/or \textbf{late binding}? \mtnote{I
am not sure we should be adopt and application-centric vision of the core
proprieties of \pilotjob implementations. This paper is about \pilotjob
...
distinction among type of bindings to be made at all.}
\msnote{I think your suggestion for scoping makes sense. I would be happy to
"implement" that.}

\item \textit{Resource exposure:} What knowledge (if any) does the application
need about the actual \textbf{resources}/\textbf{infrastructrure} where to
run? \mtnote{See the comment above about an application-centric approach.}
