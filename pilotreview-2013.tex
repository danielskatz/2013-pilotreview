...
\jhanote{I know the onus is on me to say/be specific, but I somehow am
  ...
  choices here: either we find out one or more \pilotjob systems that
...
\jhanote{The notion of a batch system should not be central to
    the definition of a job, IMO} \mtnote{Done.}
...
\alnote{I am struggling a bit with the term deployment for allocating
  ...
  environment, input/data dependencies, as well as resources.''} \mtnote{+1}
...
\jhanote{I think the previous paragraph can be removed. It is not
  central to what we need to develop in this section} \mtnote{done.}
...
\begin{description}
  \item[Placeholder.] A proxy for a bounded set of resources of a
    given infrastructure. \jhanote{is it a proxy for set of resources
      or is it a proxy for the workload or job} \mtnote{In my mental model it is a proxy for resources. It holds the office of resources, it is a proxy between who needs resources and the resources themselves. It can have that role because the resources are first assigned to it and once it holds them it can let others use them.}
  ...
  \item[Pilot.] A placeholder running on a given infrastructure and
    capable of executing pushed and pulled tasks while managing data. \jhanote{should it be placeholder job?} \mtnote{It would be consistent but do we need it? If we go for it we will need to use also placeholder VM.} 

\end{description}

From a terminology point of view, the term `pilot' as here defined is named
differently across multiple \pilotjob systems. Depending upon context, in
addition to the term `placeholder', pilot is also named `agent' and, in some
cases, `pilotjob'. All these terms should be considered synonyms and their use
to indicate the same concept is a clear indication that a minimal and
consistent vocabulary is needed when reasoning about multiple \pilotjob 
systems.

\mrnote{Is a Pilot really the same as an Agent? This gives me the heebie-
jeebies} \jhanote{Heebie-jeebies being the technical term for uncomfortable :).
Jokes aside, I too think there is a difference between pilots and an Agent}
\jhanote{OK I see what Matteo is trying to say, which is different from what is
presented: The point is that depending upon context, a pilot is also called an
agent, a pilotjob. The statement is not that a pilot is an agent etc}
\mtnote{Yep, exactly so. I attempted a revision but please feel free to edit if
still unclear.}\\---\\
...
\mtnote{The new description of early/late binding goes here}  
...
\jhanote{I'd like to
  ...
  around. OK?} \mtnote{Yep, that was part of a old version.}
