...
\texttt{Slave Node Managers}, which handle (are?) \textit{\pilotjob
  \cus}.  The end result is a complete \textit{basic \pilotjob system}, despite the
...
Pilot resource provisioning is handled by PanDA itself and is not 
user-controllable, whereas the job-to-resource bindings are handled
manually by users.
