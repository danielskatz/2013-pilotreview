...
Task and data binding alongside data retrieval are the distinguishing capabilities of a Master-Worker execution paradigm but each \pilotjob implementation can extend such a paradigm with specific functionalities. For example, \pilotjob frameworks can offer one or more strategy to aggregate the output of each task; interdependent tasks may be performed thanks to workers capable of direct communication; and tasks can be staged into pools so to improve efficiency and overall throughput.

\pilotjob frameworks implement the Master-Worker and are typically deployed within Distributed Computing Infrastructures (DCIs) like, for example, Grids, Clouds or HPC facilities. Implementation and configuration details varies considerably but each deployment of a \pilotjob framework depends at least on: (i) The provisioning of the workers on the resources of the targeted DCI, and (ii) the establishing of one or more communication channels between a Master and its workers.

Also in this case, many capabilities are implementation and context dependent. For example, the provisioning or workers depends on the capabilities exposed by the targeted DCI while one or more master can be deployed on a dedicated infrastructure, on the workstation of the user or directly on the DCI resources.
