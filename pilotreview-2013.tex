...
\title{A Comprehensive Perspective on the Pilot-Abstraction \jhanote{either
    abstraction is singular and therefore ``the'', or we use the plural form}}
...
Multi-level or meta scheduling \jhanote{I do not think the two are
  equivalent. At least not in common usage. i.e., you can definitely do meta
  scheduling without multilevel scheduling. Please argue otherwise, else I will
  remove meta scheduling.} is one of the strategies devised to improve resource
...
umbrellas of grid computing or cloud computing\jhanote{need to be more specific
  than grid computing}, targeted the resource layer, others the application
...
competing with other users for the supercomputer scheduler.  \msnote{I would
  like to either replace 'supercomputer' with i.e. 'cluster', or make it
  explicit in the beginning of 2.1 that we talk about multiple types of systems}
...
\jhanote{I think we should use neither. the generally acceptable albeit equally
  fuzzy term is high-performance machine or high-performance computing.}
...
individual resource of each Condor resource pool. \jhanote{resource management
  could not be done on application level'' does not make sense to me. Are we
  referring to aggregation?}
...
also for third parties systems like Bosco~\cite{bosco}. \jhanote{So we are sure
  flocking came before glide-in? Also there is a bit of care needed: we're
  implying glide-in is a resource placeholder -- which is part of a pilot, and
  not necessarily a full pilot.} \jhanote{as this is historical evolution, some
  parts need to be in the past tense. Care will be needed to get the tense
  right.}
...
characteristics. \jhanote{Need consistency between PanDA (two paras from now)
  and PanDa}
...
implementation. \jhanote{what is the broker here?}
...
description and, eventually, about its multiple implementations. \jhanote{would
  ``description'' be better than ``analysis'' in the first sentence?}
...
why \pilot systems allow for the direct control of workload execution and the
optimization, for example, of execution throughput. \jhanote{Based upon what is
  written, I would substitute the ``why'' with ``how'' in the previous
  sentence?}

\jhanote{pilot system are being equated with applications. sloppy} Communication
and coordination are two distinguishing characteristics of distributed
applications and \pilot systems are no exception. The Workload Manager, Pilot
Manager, and the Task Manager need to communicate to coordinate the execution of
the given workload on the instantiated resource placeholders.  Nonetheless,
\pilot systems are not defined by any specific communication pattern and
coordination strategy. The logical components of a \pilot system may communicate
using any suitable pattern (e.g. one-to-one, many-to-one, one-to-many (cit) with
a push or pull model), or coordinate adopting any suitable mechanism (e.g. time
...
protocols: different network architectures and protocols may be leveraged to
achieve effective communication and coordination. \jhanote{is this a reference
  to coordination or communication? unclear} \jhanote{in previous: mechanism or
  approach, but I would not call them strategy}
...
\jhanote{another example for the need for definitional consistency}\jhanote{also
  I would not use the word exclusive}
...
binding'.  \jhanote{we should start completing the references. currently many
  'cit' or 'ref'.}
...
within the \pilot's resource allocation.)\jhanote{full and self-contained
  sentences are generally not good candidates for parenthetical
  remarks. consider removing parenthesis?}

\jhanote{I don't think it is about simplification (that is doubtful). It is
  about greater control as a consequence} The simplification obtained as a
consequence of bypassing the job submission system of the DCI is one of the main
reasons for the success of \pilot systems. As mentioned
in~\S\ref{sec:compsandfuncs}, the tasks of a workload can be executed on a
\pilot without waiting in the queuing system of the given infrastructure, and as
a result, possibly increasing the throughput of the workload
...
both, even within the confinements of a single workload. \jhanote{Is both a
  reference to early and late binding?}
...
\pilot system based upon .... \jhanote{Matteo: please finish the sentence}.  The
terms are also mapped onto minimal set of functionalities as defined
in~\S\ref{sec:compsandfuncs} based upon .... \jhanote{Matteo: please finish the
  sentence}.

The same mapping is diagrammatically represented in
Figure~\ref{fig:core_vocabulary}. \jhanote{``The same mapping'' is unclear.}
\jhanote{Please describe figure 4 using at least 1 paragraph. Its a complex
  figure and without a detailed description it is not going to help the reader.}
...
of the task manager. \jhanote{How Table 1 helps is not at all
  clear/obvious. Hence I have removed a reference to Table 1 in the opening of
  this sentence}.
...
\jhanote{this is not needed here. we have discussed the difference between early
  versus later before. by having it here we are losing the opportunity to close
  out section 3 with the main lessons learnt} Their difference is temporal and,
as such, it can be highlighted only when describing the succession of the states
and operations of a \pilot system implementation. In the early case, the binding
of the tasks to a pilot will happen before the submission of such a \pilot to
the remote infrastructure. In the late case, the binding will happen once the
pilot has been instantiated and holds already its resources.
