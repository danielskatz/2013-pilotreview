...
\mtnote{I reviewed both definitions and I would suggest to use
  paradigm while leaving model for a technical use.}

All the \pilotjob systems described in \S2 are engineered to allow for
the execution of multiple types of workloads on Distributed Computing
Resources (DCIs) such as, for example, Grids, Clouds or HPC
facilities. This is achieved differently, depending on use cases,
design and implementation choices, but also on the constraints imposed
by the peculiarities of the targeted DCIs. Finding common denominators
among these systems require an analysis along multiple dimensions.

As seen in \S2.2.1, \jhanote{another example of why we should not use
  hard coded section/sub-section numbers. Please fix!} \pilotjobs can
be analyzed according to multiple criteria, both functional and
non-functional. In this Section, the focus is on the architectural,
provision and execution characteristics of \pilotjob
systems.\alnote{the previous characteristics are not defined yet. Are
  they suppose to get defined in 2?}\mtnote{The whole \S3.1 is
  dedicated to them but we might want to add them to the list of
  criteria in what is now 2.2.1 and will become the last subsection of
  2. What do you think?.} Other aspects as interfaces, packaging,
security, performance or scalability are here not considered because
they are not directly relevant to the comparative analysis presented
in the following Sections or because they are aspects that directly
depend on the characteristics and capabilities taken here into
consideration. \jhanote{I'm not sure I agree with the very last bit of
  the previous sentence. I think it would be more effective if we
  could use terminology defined in section 2 namely funcional or
  non-functional, and say we will discuss only functional features}

Each \pilotjob system presents architectural, provision and execution
characteristics that are either peculiar to a given system, or
distinctive of the \pilotjob paradigm itself. Isolating the latter
means to outline what distinguish \pilotjobs from other types of
distributed systems and, as a consequence, finding the minimal set of
concepts and terms that have to apply to every \pilotjob
implementation. \mrnote{"are necessary" - "have to" sounds awkward in
  this context} \jhanote{this is essentially a repeat of the previous
  paragraph; please merge/refine.}

At a high level of abstraction, for the execution of workloads all the
\pilotjob systems introduced in \S2 leverages an architecture based on
two separate but intercommunicating logical components: One related to
the management of the workloads and their tasks, the other to the
execution of the tasks on targeted resources. As it will be shown in
\S4, the implementations of these two components greatly varies from
system to system, with one or more elements responsible for specific
functionalities both at application or infrastructure
level. Nonetheless, the two architectural components can be
distinguished across multiple types of \pilotjob
systems.\alnote{should we assign a name to these two
  components?}\mtnote{\MW? ;) I see that later on you use `Workload
  manager' and `task executor'. Would you be happy with them? Would
  the others?}

For example, looking at the core \pilotjob systems introduced in
\S2.2.2\mrnote{I suggest we use section labels and not hardcode the
  section numbers - as you can see, this has already created problems
  in the first paragraph because we changed section 2.2.1 to 2.3 and
  this was not captured by this section}\jhanote{Melissa: Could you do
  the needful please?}, the scheduler and dispatcher of Nimrod-G
belong to the architectural component dedicated to workload management
while the job-wrapper belongs to the component responsible for the
execution of tasks. Similar examples can be given for the advanced
\pilotjob systems and for the Pilot-Based frameworks.  GlideinWMS
...
well-defined and self-contained architectural component --- i.e. the
one in charge of managing the workloads. \alnote{maybe mention
  explicit what these components are in the GlideinWMS case?  Also it
  would be good to explain what makes these systems
  advanced.}\mtnote{Would that not be a repetition of \S2? If we need
  more details I would add them there.} The same is valid for Corral
or SWIFT. They both offer evolved functionalities and interfaces to
manage workloads but, as such, they both are part of an architecture
where workload management and task execution belongs to two distinct
components. \alnote{Without getting into some more analysis for Corral
  and Swift, the value by stating that they have a workload manager
  and task executor is limited.}\mtnote{At this level of abstraction,
  the point to make is that those two components are there and
  separated. All the rest would be too specific. We can add details
  about Corral and Swift implementations but then we risk to repeat
  what is in \S2 or will be in \S4.} \jhanote{yes, need to be careful
  about how specific we get in order to make a point.}
...
\jhanote{I know the onus is on me to say/be specific, but I somehow am
  ...
  than a necessary choice...}\alnote{+1}\mtnote{I think the new
  version of the section addresses this concern. I see at least two
  choices here: either we find out at least one \pilotjob system that
  does not leverage a \MW paradigm or we say how it could be
  implemented differently and we give an example. I am happy to defer
  to your combined experience for both choosing the argumentative line
  and outlining a couple of examples in one or the other direction.}

Communication and coordination are two fundamental characteristics of
every distributed architecture but none of their specific paradigm is
distinctive of \pilotjob systems. For example, as seen in \S2.2.2, the
\MW paradigm is very common among \pilotjobs. Functionally, the Master
can be identified with the architectural component concerning workload
management while the Worker with the one concerning the execution of
the workload's tasks. However, the \MW paradigm usually implies a
specific distribution of capabilities between Master and Worker, and,
in some cases, also a communication model. Both these characteristics
are not necessary to implement a \pilotjob as shown, for example, by
.... and .... .\mtnote{AL/all: any suggestions to fill these dots ---
  see note above?}\mrnote{I think there are two sides to this: not
  just that M-W implies a specific distribution of capabilities but
  that M-W can be used to apply to a non-fixed set of
  capabilities. For instance, M-W is used to refer to applications,
  frameworks, architectures, etc, as such, a clear definition of M-W
  does not exist. The ellipses marks can be filled in an easier manner
  if we change the sentence to be positive from its currently negative
  state. For example, as SJ says above M-W can be ``associative'' -
  i.e. ``while the M-W concept can be applied to Pilot-Jobs, it's
  ambiguity and wide-range of capabilities make it difficult to define
  as a necessity for a PJ system.''}\jhanote{echoing what Melissa
  says, it might be more productive to introduce C-C as a sub-system
  at par with Workload Management and Execution Management rather
  than try to all inclusive/exhaustive}

\alnote{Is communication and coordination a component or a
  characteristic? (see P* paper)}\mtnote{Done.}
...
\alnote{maybe cleanly separating architecture and the way the workload
  ...
  make that separation. Care to elaborate, suggest an editing?}  The
architectural properties of \pilotjob systems support a distinctive
execution strategy \jhanote{paradigm?} with benefits for both the
compute performance and for the task submission process. The tasks of
a workload are bound to one or more pilots without having to be
directly scheduled on the DCI's job management system. Furthermore,
the binding of tasks to pilot might happen before or after the pilot
has been assigned its portion of the DCI's resources and it can
progress for the whole time the pilot can hold on them. This execution
strategy requires dedicated capabilities both for performing the
binding of tasks to pilots but also for the provisioning of pilots so
that they can be assigned portions of the DCI's resources.
...
The minimal set of capabilities that needs to be implemented by a \pilotjob system in order to execute workloads involves \pilot provision, data staging and task dispatching. \alnote{is data staging minimal?}\mtnote{if you think about it as input/output files not strictly - even if basically you would be able to run very little science without them. When you consider staging of data in general, then I believe it becomes minimal.}\mrnote{What do you mean by considering it in general? I could stage my files manually to-and-fro and only worry about the Pilots as task executors using hardcoded paths to my data. Data-staging therefore would not be required, especially on machines with shared filesystems} The procedures to provision pilots depends on how each targeted DCI exposes its resources. Typically, for a DCI adopting queue, batch and scheduler systems, provisioning a pilot involves for it to be submitted as a job. Conversely, for a IaaS, a pilot would be executed after instantiating one or more virtual machines. Data staging is used to transfer the task description and the input and output files to and from one or more pilots. Implementations of data staging vary greatly across \pilotjob implementations as much as the task dispatching procedure.\mrnote{I just want to make sure if we call data staging necessary that every PJ system we currently know of uses data staging}
...
\jhanote{I think all the functionality of PJs is predicated on the
  ...
  functionality?} \mtnote{The following paragraph is a first attempt
  to address this comment. More details will follow when late binding
  will be properly defined.}


\subsection{Terms and Definitions}

\jhanote{Idealy we should have a table of terms and a diagram showing
  their relationship. I see two ways of organizing the terms: the
  first involves definining terms based upon which of the two
  functional categories we have discussed so far they fall under, (i)
  execution management, workload management, C-C sub-systen, or (ii)
  Pilot provisioning, data staging or task dispatch.}
...
\item[Task.] A set of operations encoded into one or more applications
  that, once executed, contribute for a user to achieve a given
  goal. \jhanote{I don't think a task has to be encoded into one or
    more applications. Conversely, a single application can be
    comprised of multiple tasks. I think we should try to steer the
    definition of task towoards the workload sans any resource or
    scheduling information.}

\item[Resource.] Finite, typed and physical quantity that is consumed
  by executing one or more workload. Compute cores, memory, storage
  space, interconnects bandwidth are all examples of resources exposed
  by different types of DCIs. [cit, cit] \jhanote{how about utilized
    in lieu of consumed? Also an issue of active versus passive. Does
    the resource execute the workload, or is the workload executed on
    the resource. I prefer the latter}

\item[Infrastructure.] Structured set of resources, possibly
  geographically and institutionally separated from the users
  consuming those resources to execute one or more workload. [cit,
  cit] Infrastructures can be logically partitioned, with a direct or
  indirect mapping onto individual pools of hardware. Here
...
by a batch system. \jhanote{I think the arrow of determinism is: DCI
  determines what is available and thus if scheduler and batch systems
  are utilized.}

\begin{description}
\item[Job.] One or more program executed on a given set of computing
  resources either directly by a user or indirectly by batch system.
  \jhanote{we need to relate job to task. How is a task different from
    ...
    or set of arbitarily coordinated tasks executing on a well defined
    resource. The notion of a batch system should not be central to
    the definition of a job, IMO}
\end{description}
...
\alnote{I am struggling a bit with the term deployment for allocating
  ...
  hairs.} \jhanote{deployment and provisioning are not the same in my
...
\alnote{I would distinguish between a scheduler and resource
  manager. A scheduler is the planning component of the resource
  management system.} \mtnote{It should be done. I now use `resource
  manager system' later on.}
...
DCI.\mrnote{I am slightly uncomfortable with ``pilots are programs'' -
  maybe we can discuss this in person. I think of a Pilot as a
  relatively dumb 'container' - a PJ system would be a program to me,
  but I'm not sure of the Pilot itself, at least its unclear given my
  mental definition of pilots up til this point in the section}
...
\jhanote{I think the following paragraph can be removed. It is not
  central to what we need to develop in this section} Queuing allows
for delayed execution as it allows for jobs to be submitted when not
enough resources are immediately available for their
execution. Scheduling introduces temporal and spatial constraints on
the execution of each job so to help with maximizing resource
utilization, performance and allocation within one or more partitions
of the DCI. Commonly, such constraints include a limit on the amount
of time that a job can take to complete its execution and on the
amount of resources that it can use. Finally, batch processing imposes
to deploy all the pilots non interactively by including a job
description specifying the configuration and execution parameters for
the job's programs.\mrnote{Again, we use the word programs - I think
  ...
  we mean when we say program}

\jhanote{this paragraph should be moved into section 5} It should be
noted that cloud-based DCIs introduce notable exceptions and
...
whole \pilotjob system. \mrnote{Though you avoid queue wait, you
  introduce VM start-up time overhead as well}[cit] A similar
...
\begin{description}
  \item[Placeholder.] A proxy for a bounded set of resources of a
    given infrastructure. \jhanote{is it a proxy for set of resources
      or is it a proxy for the workload or job}
  ...
  \item[Pilot.] A placeholder running on a given infrastructure and
    ...
    stage in and out. \jhanote{should it be placeholder job?}

\end{description}

From a terminology point of view, it should be noted that the term
`pilot' as here defined is called differently across multiple
\pilotjob systems. In additition to the already said `placeholder',
pilot is also called `agent' and, in some cases,
`pilotjob'. \mrnote{Is a Pilot really the same as an Agent? This gives
  me the heebie-jeebies}\jhanote{Heebie-jeebies being the technical
  term for uncomfortable :). Jokes aside, I too think there is a
  difference between pilots and an Agent}\jhanote{OK I see what Matteo
  is trying to say, which is different from what is presented: The
  point is that depending upon context, a pilot is also called an
  agent, a pilotjob. The statement is not that a pilot is an agent
  etc}
...
terms `pilot data' or `pilot network'. \jhanote{this should move to
  much later in this section or maybe to Section 5. Must bring clarity
  to compute before opening other cans of worms}
...
portion of the resources of an infrastructure are first assigned to a
pilot and then consumed by one or more tasks. This is an important
feature of \pilotjob systems because it allows for a task to be
assigned to a pilot before it is bound to the resources. This is
why a \pilotjob system is said to allow for `late-binding' while a
traditional job-submission system of an infrastructure would allow
only for an early-binding, namely for a job to be bound to a
resource... \mtnote{discussion on the definition and distinction
  between early and late binding is needed.}
...
`binding' is defined in contrast to scheduling. \jhanote{I'd like to
  suggest that tasks are bound to a resource not the other way
  around. OK?}
...
The control exercised by the users on their placeholders depends on
the amount of information they include when submitting their tasks to
a pool. Users may fully specify how and where a task should be
executed or may leave the manager and agents to evaluate those
parameters for them. The former is called `early binding' as users
decide in advance how and where to execute their tasks, the latter is
called `late binding' because, at submission time, how and where a
task will be executed is unspecified.
...
% -----------------------------------------------------------------------------
%
\subsubsection{AppLeS -- new/restructured (compare to old below0}
\paragraph{Rationale}
...
AppLeS shows the power of the Master-Worker approach \aznote{too loaded?},
...
\subsubsection{MyCluster -- Rewritten}
\paragraph{Rationale}
...
in cited paper; verify/dig up actual applications
if this is the case}
