...
Pull vs push ~\cite{Saiz:2003:alien}.

Early apples: \cite{Berman:1996:apples}.


When Lewis Fry Richardson in 1922 devised his \textit{Forecast Factory} (
Figure \ref{fig:figures_forecast-factory}) this
might have been the first mentioning of large scale parallel computing.
In his resource estimation for quality weather forecasting, he required 64000
\textit{computers} (human beings in this case) for performing equations.
These \textit{computers} would all be assigned a part of the globe for which
they would be making their calculations by a central \textit{senior clerk}, who
was also responsible for gathering the results.
...
One common use for the \pilotjob paradigm (which has been used even before they
were named as such) is the earlier mentioned Master-Worker (M-W) scheme and
...
frameworks could respond to the dynamically changing resources by adapting the
number of workers to match the resource availability.

Around the 1996 volunteer-based computing projects started using the M-W
approach to achieve high-throughput calculations for a wide range of problems.
This also used a pull based model, as there was no central control of these
resources.
These volunteer computers can be all around the world, users merely have to
...
Although systems such as AppLeS, gave user-level control of scheduling, queue
reservation time still was an issue.
...
Singh:2008:WTC:1341811.1341822}
% \msnote{The latter is from 2008, and can hardly be considered introducing the
% concept}.
...
that all condor jobs are submitted.
