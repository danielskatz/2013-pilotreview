...
somewhat less extended discussion in 3. Would that work?}  is it intrinsicThe
...
scheduling point of each high-performance machine behind a single (meta)
scheduler. The users or the applications submit their tasks to the single
...
introduces diverse types of complexity across resources, middleware, and
applications.
...
than grid computing}, targeted the resource layer, others the application layer
as, for example, with workflow frameworks. All these approaches offered and
still offer some degree of success for specific applications and use cases but a
...
resides with the application layer. This translates into single-point solutions,
...
third-party meta-scheduler and its specific execution model. In both scenarios,
the application requires a large amount of development and capabilities that are
not specific to the given scientific problem but pertain instead to the
coordination and management of its computation.
...
meta-scheduler to schedule tasks across all the placeholders.
...
% etc.
  \centering
    ...
    implementation is used. \mtnote{Missing from from both Section 3 and 4: WISDOM, ToPoS, Corral. Missing from Section 3 but present in Section 4: MyCluster.}}
    \label{fig:timeline}
\end{figure}
...
% offer low-latency network interconnect.\msnote{This paragraph is a good
...
tense. Care will be needed to get the tense right.}
...
%``RADICAL-Pilot''~\cite{radical_pilot_paper}.
%rep- resents one of the latest evolutionary stages of the Pilot ab- straction.
%from an initial phase in which
...
of hardware (i.e. clusters, systems, and high-performance machines).
...
so as to gain exclusive control over a set of resources on one or more DCIs and
...
is provisioned on a DCI exposing specific capabilities, not a general property
...
resources of a DCI is allocated to one or more \pilots in the form of containers
supported by that DCI, and the tasks of a workload are dispatched for execution
...
% \jhanote{full and self-contained sentences are generally not good candidates
%   for parenthetical remarks. consider removing parenthesis?}
...
system of the DCI is one of the main reasons for the success of \pilot systems.
As mentioned in~\S\ref{sec:compsandfuncs}, the tasks of a workload can be
executed on a \pilot without waiting in the queuing system of the given
infrastructure, and as a result, possibly increasing the throughput of the
workload execution. Moreover, \pilots can be reused to execute multiple
workloads until the \pilots' walltime expire.  It should be noted that how tasks
are actually assigned to \pilots is a matter of implementation. For example, a
...
\pilot is inactive until it is executed on a DCI, is active thereafter, until it
...
which \pilots can be deployed. By knowing in advance the properties of the tasks
that are bound to a \pilot, specific deployment decisions can be made for that
\pilot.  Additionally, in case of early binding, other type of decisions related
to the workload could be made, e.g., the transfer of data to a certain resource
while the \pilot is still inactive. Late binding is critical in assuring the
...
without a \pilot system, but, importantly, the \pilot paradigm allows to do both
types of binding, even within the confinements of a single workload. \jhanote{Is
both a reference to early and late binding?} \mtnote{I would think so. Any
better?}
...
components of \pilot systems alongside their functionalities and the the defined
minimal and consistent vocabulary. An application submits a workload composed of
tasks to the \pilot system via an interface. The Pilot Manager is responsible
for \pilot provisioning while the Workload Manager to dispatch tasks to the Task
Manager once the pilot has been bootstrapped on the target DCI. Tasks are then
executed on the resources held by the \pilot. Figure~\ref{fig:core_vocabulary}
clearly shows the separation between the DCI and the \pilot system, and how the
...
\pilot system depends upon understanding the inclusion relation among resources,
pilots, jobs, clusters, and DCI.
...
Note how scheduling happens at both the infrastructure and at the pilot
level. In Figure~\ref{fig:core_vocabulary}, scheduling clearly happens at the
level of the resource pool of an infrastructure, for example by means of a
cluster scheduler, and then of the pilot once it holds its resources.
In Table~\ref{table:terminology} the term `multi-level
scheduling' pertains to both the pilot and workload managers and their
respective pilot provisioning and task dispatching functionalities.

Figure~\ref{fig:core_vocabulary} also help to appreciate the critical
...
pilot has been provisioned, it is the pilot and not the container to be
...
\jhanote{removed table reference..as a consequence of changes in 3, broken
  ...
  core simply as, ``... necessary for any \pilot system''?}
...
\item \textbf{Infrastructure Interaction}. \pilot systems interact with remote
  ...
  still valid?}
  \msnote{I'm still not comfortable with the overlap of this with Task
  Execution and Pilot Deployment}
  \mtnote{I am OK with it but if you offer an alternative I will see how to integrate it into the existing text.}
...
functionalities.\msnote{Richer than what? Not sure how I should interpret this
sentence} \mtnote{Better?}
...
of why we want to run MPI with pilots.}\msnote{Actually these situations are
more challenging when running on \pilots, as the different phases often have
different computational requirements.} \mtnote{So why we do it?}
...
effective.\msnote{The previous sentence might undercut our story more than we
wish, as all existing systems started as that, but consequently ran into trouble
when trying to grow beyond that}\mtnote{I am not sure how this undercuts `our
story'. Your comment seems consistent with the previous paragraph: if what you
need is just a special purpose system/single point solution then go for a
specialized \pilot system. When you need flexibility and generality, go for the
multi-purpose, functionally encompassing one maintained by an open community.}
...
systems. I would probably not do it as it does not seem of primary relevance to
me.}
...
This work has also been made possible thanks to computer resources provided by
TeraGrid TRAC award TG-MCB090174.
