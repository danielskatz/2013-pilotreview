...
\jhanote{The 5th and 6th paragraphs should be moved. It is
interrupting the flow and delaying the reader from getting to the
message. 5th and 6th are useful but we should present
caveats/enhancements/refinements to a message only after we have
finished delivering the main message. For example, we can move the
discussion of C\&C to where we talk about data management.}
...
\msnote{Do we need "examples" here in the first place?}

Communication and coordination are two \msnote{I feel we need an adjective
here, otherwise it reads a bit like "two randomly choosen characterics are ...
...
\msnote{Arguably, the "definition" of the logical components will restrict to
some degree which functionality belongs here and there? At least not in general
I would say. But maybe this statement is still too ambigious because we haven't
talked about "functionality" yet?}
...
\msnote{submission is confusing here} and a
...
workload.
\msnote{We just describe what can't be done without them. Wouldn't it be better
to (addionally) give a definition? Or was this approach deliberately chosen?}
...
\msnote{What do we strictly consider the process of "pilot provisioning" to
entail? In some definition, this would rule out DIANE as a pilot job framework
as the launch of pilots is done out-of-band.}
This type
...
cit]\msnote{What are we thinking of here?}.
...
\msnote{Dispatch in this context has quite a literal meaning of "sending", and
would kind of rule out pilots "pulling" tasks.}
