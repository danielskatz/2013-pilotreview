...
properties are necessary for every \pilot system implementation to provide the
minimal set of components and functionalities as described
in~\S\ref{sec:compsandfuncs}. Auxiliary properties are instead required only as
a support to the implementation of core properties. For example, while every
\pilot system has to implement pilot deployment, a specific authentication and
authorization protocol may be required to deploy pilots by means of a particular
DCR middleware. As such, auxiliary properties are not necessarily shared among
all \pilot systems.
...
minimal and complete while that of auxiliary properties is an representative
subset. While a minimal set of properties has to characterize a \pilot system in
order for it to implement the components and functionalities described
...
The Pilot Manager implementation requires at least the following in order to
provide its Pilot Provisioning functionality: (i) managing pilots holding at
least one type of resource in order to be used to execute tasks; (ii) pilot
wrappers of the type required by the target DCR so to make possible for a pilot
to be scheduled on the chosen DCR; (iii) bootstrapping environment and routines
so that pilots can become available for task scheduling and execution. These
features are here grouped into two core properties: Pilot Resources and Pilot
Deployment.

The Workload Manager implementation needs at least the following to provide its
...
(if any) to make possible for a workload to be scheduled and then executed; (ii)
pilots with a list of tasks and each task listed in at least one pilot so to
enable complete binding between task and pilot(s); (iii) task lists associated
with a pilot description or to a pilot instance in order to obtain respectively
early and late binding. Workload Semantics and Workload Binding are the two core
properties used to represent these Workload Manager features.
...
implemented as the execution of a task would fail, for example due to
misconfiguration or lack of required supporting software. This requirement is
here represented with the core property named `Workload Execution'.
