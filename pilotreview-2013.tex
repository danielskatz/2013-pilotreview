...
italicizing or even boldfacing. Should we change the ``implementations'' to just
systems, as currently the reader might be confused between implementations and
individual instantiations?} \mtnote{Done.}
...
implementation and usability of the core properties. While these
properties might be necessary for a \pilot implementation, in of
...
authentication, authorization, and accounting; support for multiple usage modes
like HPC or HTC; or robustness in terms of fault-tolerance and
high-availability.  \jhanote{alternate style suggestion: moved the earlier last
  sub-sentence to the beginning. i've commented out the original text in case
  you'd like to revert} \mtnote{I reviewed the second sentence because it was incomplete.}
...
\jhanote{the transition to 4.1.2 is a bit unsmooth, e.g., no text before we
  ...
  into a 4.1.1 component and a 4.1.2 component?} \mtnote{Went for option (i). Better?}
...
moment we are not using them but please let me know whether you think we should
go back to them.}
...
\jhanote{logical?} \mtnote{I am not sure we need to distinguish among types of
  ...
  explicitly calling them as logical} \mtnote{Added logical to the other pilot descriptions.}
...
\jhanote{I'm not sure the meaning of the previous sentence is clear: does it
  ...
  of correctness. OK?} \mtnote{I think there is loss of meaning: if I got the literature right, it was about getting HTCondor to work with grid middleware. Any specific reason because using `grid middleware' is problematic?}
