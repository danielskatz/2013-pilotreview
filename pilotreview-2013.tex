...
Isolating them means to outline the minimal set of concepts and terms
...
\jhanote{I have no idea how the later part of this paragraph (starting
  ``Communiction and coordination..'') has become duplicated.  Is the
  intent for the next paragraph to be the actual paragraph?}  The
Workload Manager and the Task Executor are separate and well-defined
architectural components that communicate and coordinate in order to
exchange tasks, input and output files, and related data.
...
\S\ref{ssec:informal}, the \MW paradigm is very common among
...
% While communication and coordination are two fundamental
...
\mrnote{I suggest we use section labels and not hardcode the section
  numbers - as you can see, this has already created problems in the
  first paragraph because we changed section 2.2.1 to 2.3 and this was
  not captured by this section} \jhanote{Melissa: Could you do the
  needful please?}
...
\item[Resource.] Finite, typed and physical quantity utilized when
  ...
  running workloads \jhanote{Also an issue of active versus
