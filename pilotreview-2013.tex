...
umbrellas of grid computing or cloud computing\jhanote{need to be more specific
  ...
  thinking about? Grid and cloud computing are at the same level of generality
  so I will have to specify also the latter.}, targeted the resource layer,
...
robust abstractions has still to be devised and implemented.\jhanote{sorry my
  remark wasn't very clear: i meant we need citations rather than a simple
  reference to grid and or cloud computing. Either upstream or with ``all
  these..."}
...
interoperability. \jhanote{maybe same case here. specific examples/citations as
  opposed to ``grid computing''}
...
\jhanote{first sentence needs attention} The notion of resource placeholder was
devised as a pragmatic and relatively cheap to implement attempt to reduce or at
least better manage the complexity of executing distributed applications. A resource placeholder decouples the acquisition of remote compute resource from
their use to execute the tasks of a distributed application. Resources are
acquired by scheduling a job onto the remote high-performance machine. Once
executed, the job runs an agent capable of retrieving and executing application
tasks.
