...
introduced by Y.) \msnote{find a nice place for this line}
...
\msnote{I didn't write that line :-) See my earlier commented out comment. We
still need a bridging sentence between apples and pilots}.
...
\mrnote{What are core PJ systems? Core pilot job systems = those that provide
only simple pilot capabilities? What are simple pilot capabilities? Not sure
this has been established.}
\msnote{Fair point Melissa. I agree in general, but we (will) have an informal
definition in section 1 that we can refer to here I think. I did rewrite it
already a bit to address your concern though.}
...
among these systems requires the definition of the \pilotjob paradigm
along multiple dimensions: purpose, logical components, and
functionalities.
...
The purpose of a \pilotjob system is to improve performance of workload
execution when compared to executing the same workload directly on one
or more  DCI. In order to achieve such a purpose, each \pilotjob system
exhibits characteristics that are both integral and not-integral to the
\pilotjob paradigm itself. Discerning and distinguishing these
characteristics require isolating and defining the minimal set of logical
components that characterize the \pilotjob paradigm.

At some level, all \pilotjob systems introduced in \S\ref{sec:history}
leverage three separate, but coordinated logical components: a
\textbf{Pilot Manager}, a \textbf{Workload Manager}, and a \textbf{Task
Executor}. The Pilot Manager handles the definition, instantiation, and
use of one or more resource overlay called `pilot' on single or multiple
DCIs. The workload manager handles the inspection, submission and
retrieval of one or more workload on the available resource overlays.
Finally, the Task Executor takes care of executing the tasks of each
workload on one or more resource overlays.
...
Nevertheless, the three components can be consistently distinguished
across different \pilotjob systems. For example, looking at the core
\pilotjob systems introduced in \S\ref{ssec:history}, \ldots \mtnote{I
will wait for S2 and S4 to be final before picking the right
examples/vocabulary.}
...
The logical components of the \pilotjob paradigm support a minimal set
of functionalities that allow for the execution of workloads by means of
pilots: \textbf{\pilot provisioning}, \textbf{task dispatching}, and
\textbf{task execution}. Every \pilot system need to be able to schedule
\pilotjobs on the targeted resources, schedule tasks on the available
pilots and then leverage these \pilots to execute the tasks of the given
workload. While many more functionalities are needed to deploy,
manage, or share a \pilot system among one or more users,
functionalities related to security, workload submission, resource
description, or data management are, strictly speaking, not required
for \pilotjobs or tasks to be scheduled and then executed.
...
because it is essential for the creation of resource overlays. This type
...
each \pilot system. Typically, for a DCI adopting queues, batch and
...
container as, for example, a container implemented by means of a Virtual
...
and can be implemented within the boundaries of the \pilot systems. This
independence allows to shift the control of tasks of a workload directly
and exclusively to the \pilot system, a distinguishing characteristic of
such systems.

The distribution of functionalities among the Pilot Manager, Workload
Manager and the Task Executor depends on the communication patterns and
coordination strategies chosen to implement a \pilot system. As such,
communication and coordination should not be used to define the \pilot
paradigm. For example, as seen in \S\ref{ssec:informal}, \MW is a very
common cooridnation strategy among \pilot systems. When the Master is
identified with the Workload Manager, and the Worker with the Task
Executor, the functionalities related to task description, scheduling,
and monitoring will generally be implemented within the Workload
Manager, while the functionalities needed to execute each task will be
implemented into the Task Executor. Alternative coordination strategies,
for example the one where a Task Executor directly coordinates the task
scheduling, might require a functionally simple Workload Manager but a
...
neighboor executors leveraging, for example, an overlay network.
...
exmaple, the Workload Manager and the Task Executor may communicate with
every suitable pattern --- point-to-point (i.e. one-to-one), many-to-
one, or one-to-many (cit) --- and coordinate adopting any suitable
strategy --- time syncronization, static or dynamic coordinator
election, local or global information sharing. Analogously, different
network architectures and protocols may be leveraged in order to achieve
...
capabilities offered by specific DCI. Nonetheless, \pilot systems can be
devised in which tasks do not require data management because they (i)
do not necessitate input files, (ii) do not produce output files, (iii)
data is already locally available or (iv) data management is left to the
application itself. As such, data management should not be considered a
necessary functionality of \pilot systems even when present in many of
them.
...
\msnote{I dont think a job is actually a container of executables}
\msnote{Should we maybe relate job to tasks? Although I guess we do that in the
...
provisioned.
\msnote{I dont understand the part of "the sum" and the following example ...}
...
\msnote{I think I start to understand the earlier definition of "container
sum", but we might want to clarify that earlier}
...
\msnote{Im not sure if it is more reliable, but we can add or replace with
"flexible"?}
...
\msnote{The "first and then" is possibly in conflict with the next sentences.}
...
\msnote{When a pilot is still in a queue, waiting to be run, it is inactive,
but binding to this pilot would be considered late, right? (because the choice
for the resource has already been made)}
...
resource.
\msnote{The "specified" part here probably requires some clearification.}
