...
% Other aspects as interfaces, packaging, security, performance or scalability
% are here not considered because they are not directly relevant to the
% comparative analysis presented in the following Sections or because they are
% aspects that directly depend on the characteristics and capabilities taken 
...
\jhanote{another example of why we should not use hard coded section/sub-
section numbers. Please fix!}\mtnote{I do not have the lock on Section 2 so I cannot add the references to those Sections and Subsections. If the owners of that sections add the anchors I will be happy to use them here.}

%\jhanote{I'm not sure I agree with the very last bit of
...
%  non-functional, and say we will discuss only functional features}\mtnote{
%  done}\\---\\

%\mrnote{"are necessary" - "have to" sounds awkward in this context}\mtnote{Done.} 
%
%\jhanote{this is essentially a repeat of the previous paragraph; please 
%merge/refine.}\mtnote{Done.}\\---\\

From a high level perspective,  all  \pilotjob systems introduced in \S2
leverage a similar architecture based on two separate but intercommunicating
logical components to accomplish workload execution:  one is related to the
...
these two components significantly vary from system to system, with one or more
elements responsible for specific functionalities both on application as well as
...
\alnote{should we assign a name to these two
...
\S2.2.2, the scheduler and dispatcher of Nimrod-G
belong to the architectural component dedicated to workload management
while the job-wrapper \onote{job-wrapper seems to be an arbitrary term?} belongs to the component responsible for task execution. Similar examples can be given for the advanced
...
one in charge of managing the workloads. The same is valid for Corral
...
\mrnote{I suggest we use section labels and not hardcode the
...
\alnote{maybe mention
  ...
  more details I would add them there.} 

\alnote{Without getting into some more analysis for Corral
...
\onote{Flow interrupted w.r.t., prev. paragraph}
...
a distinctive feature of \pilotjob systems. For example, as seen in \S2.2.2, the
...
in some cases, also a communication model. Moreover, \MW is applied to a non-fixed set of capabilities while referring not only to architectures but also to frameworks and applications. This flexibility makes \MW a viable choice for implementing \pilotjobs but not one of its defining characteristics. After all, \pilotjobs system with an highly capable task executors and workload managers could be easily conceived. 
...
\jhanote{I know the onus is on me to say/be specific, but I somehow am
  ...
  choices here: either we find out one or more \pilotjob system that
...
\mtnote{AL/all: any suggestions to fill these dots ---
  ...
  as a necessity for a PJ system.''}\mtnote{I have incorporated your comment in the paragraph above. Please note that a closure is still needed as it is not strong enough to argue that \MW is too ambiguous to be necessary. A simple rebuttal would be: make it less ambiguous and you will see that it is indeed necessary because, after all, there is no plausible alternative. Is the current offering for a plausible alternative satisfactory? Please feel free to add/change/edit directly the paragraph if not.}

\jhanote{echoing what Melissa
  ...
  than try to all inclusive/exhaustive}\mtnote{This is still in progress. To summarize the checkpoint of our discussion: assigning Communication/Coordination to a sub-system would mean to move C-C into an architectural component of PJ. In so doing we would incur into an over-specification of the minimal PJ architecture. After all, C-C does not necessarily need to belong to an independent, well-defined architectural component of PJ, it could be a feature implemented in the Workload manager and the task executor. The paragraph opens stating the fundamental nature of C-C and goes on explaining that no specific pattern for both Cs is required.}\\---\\

The architectural properties of \pilotjob systems support a distinctive
...
\alnote{Is communication and coordination a component or a characteristic? (
see P* paper)}\mtnote{Done.}
...
one or more virtual machines. Data staging is used to transfer the task
...
therefore would not be required, especially on machines with shared filesystems
}\mtnote{Apologies, my comment was too generic. I mean that it is not only about staging I/O files, it can also be about staging the pilot/agent --- BJ auto-install into a venv for example --- or downloading the the executable used for the task. If we take away the very possibility to move data --- whatever function they play into the execution `model' of PJ --- in and out I think that the PJ system as a whole cannot work.}
...
\mtnote{The following list is still to be updated following a complete list of 
the definitions offered in the next subsection. I am going to do it towards 
the end of the revision process so to avoid multiple rewrite of the list.}
\begin{itemize}
...
\jhanote{I think all the functionality of PJs is predicated on the
  ...
  functionality?} \mtnote{The previous paragraph is a first attempt
...
\jhanote{Ideally we should have a table of terms and a diagram showing
  their relationship. I see two ways of organizing the terms: the
  first involves defining terms based upon which of the two
  functional categories we have discussed so far they fall under, (i)
  execution management, workload management, C-C sub-system, or (ii)
...
\mrnote{I am slightly uncomfortable with ``pilots are programs'' -
  ...
  mental definition of pilots up till this point in the section}\\---\\
...
Queuing allows for delayed execution as it allows for jobs to be submitted when
not enough resources are immediately available for their execution. Scheduling
introduces temporal and spatial constraints on the execution of each job so to
help with maximizing resource utilization, performance and allocation within 
one or more partitions of the DCI. Commonly, such constraints include a limit  
on the amount of time that a job can take to complete its execution and on the
amount of resources that it can use. Finally, batch processing imposes to 
deploy all the pilots non interactively by including a job description 
specifying the configuration and execution parameters for the job's programs.

\jhanote{I think the previous paragraph can be removed. It is not
  central to what we need to develop in this section} 
...
and executing tasks that consume those resources. Other two fundamental 
capabilities of a pilot are pulling or being push with the tasks to executed, 
...
\mrnote{Is a Pilot really the same as an Agent? This gives
...
one of the main responsible for the success of the \pilotjob systems. The 
...
\mtnote{discussion on the definition and distinction 
