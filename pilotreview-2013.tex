...
exposed by a single cluster of the DCI or by a DCI-wide scheduler.
...
both to perform the binding of tasks to pilots but also to
provision pilots so that they can be assigned portions of any of a
DCI's resource.
...
workload. Pilot provisioning and task dispatching should be considered
...
Executor depends on the communication patterns and coordination
strategies chosen to implement a \pilot system. Therefore, such a
distribution should not be used to define the \pilot paradigm. For
example, as seen in \S\ref{ssec:informal}, \MW is a very common
cooridnation strategy among \pilot systems. When the Master is
