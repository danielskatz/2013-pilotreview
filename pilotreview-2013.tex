...
the \pilot is still inactive. 
Late binding is critical in assuring the aforementioned high
...
backfilling but to scheduling tasks to an active pilot. There is no queuing
...
throughput.}\amnote{All else being equal, isn't the difference then that 
the pilot is submitted earlier rather then the effect of early vs. late 
binding?  Assume a pilot is submitted a $T_0$, tasks are submitted at $T_1$, 
the pilot becomes active at $T_2 > T_1$.  On both early an late binding, the
tasks can only start execution after $T_2$ -- what shifts is time of
scheduling, not time of task execution.  For a single pilot, it should not 
make a difference.} 
It should be noted that some aspects of early binding can also be
