...
the systems mentioned in \S\ref{ssec:history}, \ldots \mtnote{I will
...
queues, batch and scheduling systems, provisioning a placeholder
involves it being submitted as a job. A `job' on a Grid middleware is a
type of logical container that wraps configuration and execution
parameters alongside information on the kernel that will need to be
executed by the operating system of the DCI compute nodes. Conversely,
for infrastructures that do not adopt a job-based middleware, a resource
placeholder would be executed by means of other types of logical
container as, for example, a Virtual Machine (VM) or a Docker Engine
(cit, cit).
...
not depend on other functionalities provided by the DCI and can be
implemented within the boundaries of the \pilot system. This
independence shifts the control of the workload execution directly and
exclusively to the \pilot system. Among the most relevant consequences
of such a shift  is decoupling execution time from the queuing time
imposed by the DCI. Equally relevant, complex execution patterns
involving task and data interdependence can be utilized without having
to deal with the constraints imposed by the DCI middleware. Ultimately,
this is why \pilot systems allow for the parametrization of workload
execution and the maximization and minimization of one or more execution
metrics.
...
coordinate all the necessary steps needed to execute the given workload.
Analogously, the workload and pilot managers need to communicate in
order to coordinate the instantiation and utilization of resource
...
example, functionalities can be provided to support data staging for
task execution or for managing task-related data according to the
capabilities offered by specific DCI. \pilotjob systems can be devised
in which tasks do not require any data management because they (i) do
not necessitate input files, (ii) do not produce output files, (iii)
data is already locally available or (iv) data management is left to the
application itself. Even if such systems are conceivable, for every
practical reasons, a minimal set of data-related capabilities should be
consider a necessary functionality of a \pilotjob system. Therefore,
while staging input files in and out a resource has to be considered a
necessary capability of every \pilotjob systems, the same does not apply
to more advanced and specific data capabilities like, for example, data
replication, concurrent data transfers, data abstractions different from
a file, or pilot data.
...
\item[Task.] A set of operations encoded so that they can be executed on
the resources of a DCI. Implementations of a task may include wrappers,
applications, scripts, and, more in general, any type of `application
kernel'.

\end{description}

As seen in \S\ref{ssec:informal}, most of the DCIs where \pilotjobs
systems are executed utilize `queues', `batch systems' and `schedulers'.
In such DCIs, jobs are scheduled and then executed by a batch system.
