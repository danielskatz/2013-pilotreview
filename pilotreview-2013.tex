...
production sites. Other schedulers enable AutoPilot to submit to local and remote
...
in~\S\ref{sec:understanding}. The Pilot Manager and the Workload Manager are
implemented by the CU Manager. The Agent is deployed on the DCR to expose its
resources and execute the tasks pushed by the CU Manager. As such, the Agent is
a pilot.
...
destination. \jhanote{need to add reference to the figure..}
...
class systems like NERSC Hopper, OLCF Titan, or NCSA Bluewaters. Adding support
for a new DCR is relatively simple, as proven for example with the LRZ SuperMUC
Petascale System or the recent inclusion of a connector for public and private
IaaS DCRs. \jhanote{not sure we want to mention hopper, titan and bluewaters or
  supermuc as they are likely to become ``dated'' soon. thoughts?}
