...
to be named inconsistently\msnote{Inconsistently could mean here that the systems
are not consistent themselves, but I think we try to convey is that the systems
are not consistent with each other}, often with the same terms referring to multiple
...
we just about established that the terminology is not shared? :-)}. The goal is
...
\onote{Sometimes we use 'logical' components and sometimes 
'architectural components' throughout this section and others. 
I think logical components should be used consistently and the 
term 'architectural components' should be avoided all together.}


All the \pilotjob systems introduced in \S\ref{sec:prelim} are engineered to
allow for the execution of multiple types of workloads on Distributed
Computing Infrastructures (DCIs) such as, Grids, Clouds or HPC facilities. This is
...
reference?}, \pilotjobs can be analyzed according to
...
the \pilotjob paradigm\msnote{I like the concept "pilot-job paradigm", it might
be a good "general" term to use} itself. However, many \pilotjob systems also exhibit
...
two-way on purpose?} logical components: a
...
architectural component dedicated to workload management, while the job-wrapper
...
paradigm a defining feature of a \pilotjob system.\msnote{How about changing
the last words into: "... the \pilotjob paradigm."?}
...
exposed by the targeted DCI. Pilots are bound\msnote{scheduled?} to the DCI resources by means of
the DCI capabilities but, once bound, pilots make those resources directly
available for the execution of the tasks of a workload.\msnote{Arent the
resources only available after the pilot becomes "running"?}
Thanks to resource overlays, \pilot systems can also implement functionalities tailored to the
...
require data management because they (i) do not necessitate input files, (ii) do
not produce output files, (iii) data is already locally available or (iv) data management
is left to the application itself. As such, data
...
\item \textit{Resource Access}: How does the Workload Manager interface with 
the physical resources, i.e., with the middleware that provides resource access.
...
\paragraph{Task Dispatching}

Task dispatching is the second of two core capabilities we 
have defined for \pilotjob systems in section~\ref{subsec:vocab_core_functionalities}.
Both logical components -- workload manager and task executor -- are concerned 
with the implementation of task dispatching (see Table{~\ref{table:terminology}). 
The classifiers listed here help to distinguish how task dispatching is 
implemented and what the distinct feature of a specific implementation are. 


\begin{itemize}

\item \textit{Single or Multi-User Task Pool}
