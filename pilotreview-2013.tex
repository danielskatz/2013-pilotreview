...
\jhanote{melissa: in addition to tps the total number of tasks
  executed is also an important consideration sometimes}
...
\jhanote{melissa: this sentence is awkward. please fix}in which more
processors were assigned to jobs with longer runtimes, so as to make
the simulation take the same amount of wall-clock time between points
when they need to communicate with one another (via MPI).

\jhanote{melissa: this paragraph needs major rewriting. unclear what
  the message/aim is} Some simulations require varying number of tasks
to complete -- that is, 10 tasks may start a program but the results
of these tasks may spawn more tasks to complete. \pilotjobs can be
utilized for these types of dynamic simulations, because once they are
active, the agents have an available slot for executing tasks on a
given number of cores. As more tasks are added to the system, the
\pilot can continue to execute new tasks until the job is considered
complete. Without \pilotjobs, these simulations would have to be
resubmitted to the job queue and wait for their time to become active
again~\cite{luckow2009adaptive}.
...
end-user. In contrast to GlideinWMS these Glide-Ins\jhanote{substitute
  with Corral Glide-Ins?} are run using the credential of the user and
...
\alnote{Should we discuss MOTEUR?}
...
The functionalities of each \pilotjob framework can be broadly divided into two categories: One related to the execution of workloads the other to the deployment of the pilot machinery. 

\pilotjobs leverage a Master-Worker paradigm [ref] for the execution of workloads. In such a paradigm, every given workload is reduced into tasks and each task, alongside the data needed for its execution, is bound by a master to one or more workers. Once the tasks have been executed, each worker makes available the results of the computation to the master. 

Task and data binding alongside data retrieval are the distinguishing capabilities of a Master-Worker execution paradigm but each \pilotjob implementation can extend such a paradigm with specific functionalities. For example, \pilotjob frameworks can offer one or more strategy to aggregate the output of each task, interdependent tasks may be performed tanks to workers capable of direct communication, and tasks can be staged into pools so to improve efficiency and overall throughput.

\pilotjob frameworks implement the Master-Worker paradigm. These frameworks are typically deployed within Distributed Computing Infrastructures (DCIs) like, for example, Grids, Clouds or HPC facilities. Implementation and configuration details varies considerably but each deployment of a \pilotjob framework depends at least on: (i) The provisioning of the workers on the resources of the targeted DCI, and (ii) the establishing of one or more communication channels between a Master and its workers.

Also in this case, many capabilities are implementation and context dependent. For example, the worker provisioning depends on the capabilities exposed by the targeted DCI while one or more master can be deployed on a dedicated infrastructure, on the workstation of the user or directly on the DCI resources.

Task and data binding, data retrieval from the workers, workers provisioning and master/worker communication are the defining capabilities of each \pilotjob framework. In the following two subsections, a minimal set of terms related to these capabilities is defined. The terms are sorted depending on the category of capability to which they belong. The end result is a coherent vocabulary that can be effectively adopted across multiple \pilotjob implementations. 

Here a summary of the terms defined in the following subsections and the category of capabilities to which they belong.
...
\onote{'data' is not mentioned at all in the context of Jobs. Why? }
...
In order for a \pilotjob framework to implement a Master-Worker execution paradigm, workers needs to be implemented and provisioned on the targeted DCI(s). This is done through the capabilities exposed by each DCI in order to access and consume its resources. As seen in \S 2, most of the DCIs where \pilotjobs frameworks are deployed adopt an execution model based on `jobs', `schedulers' and `batch systems'. In such DCIs, jobs are scheduled by a user to be executed by a batch system on the DCI resources.

\begin{description}
  \item[Job] One or more programs that are executed on a given set of computing resources either directly by a user or indirectly by batch processing.
\end{description}
...
The DCIs where \pilotjobs framework are usually deployed, utilize a scheduler to manage the submission of jobs by multiple users. The scheduler introduced temporal and spatial constraints on the execution of each job in order to maximize resource utilization, performance and fairness. Commonly, such constraints include a limit on the amount of time that a job can take to complete its execution and a limit on the amount of resources that it can use.

Finally, many of the DCIs where \pilotjob frameworks are deployed leverage also a queuing system that allows for jobs to be submitted  when not enough resources are immediately available for their execution.

All these features and capabilities are relevant when considering the deployment of \pilotjob frameworks because, in the context of these DCIs, every worker of the framework is a job on the DCI. When present, queuing, scheduling or batch processing define the practical boundaries for how and when the workers of a \pilotjob framework can be deployed. Queuing introduces delays, scheduling imposes limits on for how long a worker can be available to its master and on how many resources can consume, while batch processing imposes a non-interactive deployment of the workers. 
...
Cloud-based DCIs introduce notable exceptions and differences in the way in which the workers of a \pilotjob framework can be deployed.[cit, cit] In a IaaS [cit] context, Virtual Machines (VMs) and not jobs are used to deploy one or more workers. VMs can often be instantiated directly, without waiting into a queue, and are not necessarily subjected to walltime limitations. Clearly, overheads are introduced by having to deal with VMs and not `simple' jobs and with the model adopted within a IaaS-based DCI to assign resources to each VM.[cit] It should be noted that a Platform as a Service (PaaS) [cit] approach to cloud computing is in principle compatible with a Master-Worker execution paradigm and, as such, a dedicated \pilotjob framework could be implemented to leverage the benefits offered by PaaS.[cit]

Showing that the deployment modalities of a \pilotjob framework depend on the capabilities exposed by the targeted DCI illustrates the limits of choosing the term `job' to identify a worker. The use of the term `job' is due to an historical contingency, the targeting of a specific class of DCIs in which the term `job' is meaningful. With the development of new types of DCI, the term `job' has become too restrictive, a situation that can trigger confusion and the use of synonyms like `agent' or `pilot'.
...
\begin{table*}[t]
 ...
 \begin{tabular}{|p{3cm}|p{3.2cm}|p{3cm}|p{3cm}|p{3cm}|}
  ...
  \hline
          PanDA             & Automatic, Not user-controllable         & --                                 & --                          & --                     \\ 
  \hline
 ...
 \hline
          BigJob             & Manual, Explicitly User-controllable     & Manual (rule-driven) / Automatic   & Multi / Single              & --                     \\ 
 \hline
 \end{tabular}
 \caption{\textbf{Different pilot-job systems and their key. 
 \onote{Possible classifiers:
 user/system space: can pilots be controlled from system / user space. }}
 \up} 
 \label{table:bigjob-saga-diane}
\end{table*}
...
multi-level scheduling.  \aznote{Lack of multi-level scheduling asserted because even
...
MyCluster implements several \pilotjob analogues:
the \texttt{Master Node Manager}, which acts as a \textit{\pilotjob Manager},
the \texttt{Task Manager}, which acts as an \textit{\pilotjob Agent}, and 
\texttt{???}, which are the equivalent of \textit{\pilotjob \cus}.
...
BigJob is a Pilot-based framework which provides a higher-level, unifying interface to heterogeneous and/or distributed data and compute resources. The framework is accessed via the Pilot-API, which provides two key abstractions: Pilot-Job and Pilot-Data. BigJob eliminates the need to interact with different kinds of compute resources, for example, batch-style HPC\/HTC as well as cloud resources. In order to communicate with these resources, BigJob uses the Simple API for Grid Applications to submit a Pilot-Job to a variety of different backends -- including PBS, SGE, Condor, and SLURM.

\textit{Pilot Manager} orchestrates a set of Pilots running on the submission machine. This is the central coordinator of the framework.
\textit{Pilot Agent} is a decentralized component which manages the set of resources assigned to it.
\textit{Distributed Coordination Service} is used for communication between the Manager and the Agents.
