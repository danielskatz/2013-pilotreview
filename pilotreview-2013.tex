...
in the context of the LHC data challenge.\alnote{the concept or the name? figure 2 goes back way further than 2004} It's first written appearance is in a
2005 LHCb report\cite{lhcb2005}). \msnote{find a nice place for this line}
...
\jhanote{Mark: I propose the following:
One common use for the M-W scheme is to serve as the coordination substrate for
PJs. I know its a bit nebulous, but it connects the two concepts directly,
...
%On the remote resource a set of Condor daemons is started, which then registers
%the available job slots with the central Condor pool.
...
%Glide-in installs and executes necessary Condor daemons and configuration on
%the remote resource, such that the resource reports to and joins the local
%Condor pool.
...
%several other systems emerged.
% Commented out gwpilot. Not really published/downloadable, and no
% obvious new functionality.
%GWPilot~\cite{gwpilot} is a \pilot system that is developed as a component for
%the GridWay meta-scheduler.
%GWPilot emphasizes its easy deployment, multi-user support and the support of
...
%the Globus GRAM service.
%The provisioner spawns a set of executor processes on the allocated resources,
%which are then responsible for managing the execution of task.
%Tasks are submitted via a so called dispatcher service.
...
%environment for supporting drug discovery.
%The architecture utilizes an agent run as a Grid job to pull tasks from a
...
% Both will be leveraged in
% \S\ref{sec:4} and \S\ref{sec:5} to produce a comparative and critical
% analysis of diverse \pilotjob frameworks.

% \jhanote{not sure about first goal, ``an exemplar \pilotjob system''?}
% \mtnote{Any better?}

% \jhanote{forward reference, a matter of style, but either way must be
% conservative with its usage}
% \mtnote{Any better?}

% \jhanote{The 5th and 6th paragraphs should be moved. It is interrupting
% the flow and delaying the reader from getting to the message. 5th and
% 6th are useful but we should present caveats/enhancements/refinements to
% a message only after we have finished delivering the main message. For
% example, we can move the discussion of C\&C to where we talk about data
% management.} \mtnote{I revised the two paragraphs and moved them down.}
...
% As seen in \S\ref{ssec:evaluation} \pilotjobs can be analyzed according
% to multiple criteria, both functional and non-functional.  \onote{avoid
% functional v.s. non-functional} In this section, the focus is on the
% former as the latter are adapted by definition to each specific pilot
% implementation.
...
% the scheduler and dispatcher of Nimrod-G \jhanote{this is now a stale
% statement, as Nimrod is not discussed in Section\ref{ssec:history}}
% belong to the logical component dedicated to workload management, while
% the job-wrapper belongs to the component responsible for task execution.
% Similar examples can be given for the advanced \pilotjob systems and for
% Pilot- based frameworks: GlideinWMS \jhanote{we'll need to be careful to
% not conflate a WMS with a PJ systems. It may be just a matter of naming,
% ie replace GlideWMS with Condor-G, but needs care either way..} offers
% an extended set of features when compared to the scheduler and
% dispatcher of Nimrod-G, but all those features belong to the same, well-
% defined and self-contained logical component --- i.e. the one in charge
% of managing the workloads. The same is valid for Corral and
% \jhanote{Need to replace Swift with Coasters -- Swift is a workflow
% language; coasters is the underlying pilot implementation.} Swift: both
% offer evolved functionalities and interfaces to manage workloads but, as
% such, both are part of an architecture where workload management and
% task execution belong to two distinct components. \jhanote{fix previous
% sentence}\mrnote{I think this paragraph may need an entire revisit,
% since comparison  with Nimrod is no longer an option, swift is not a PJ-
% system, and glidein is compared vs nimrod}

% ranging from `dumb', `intelligent', or overlay networks,

% \jhanote{suggestion: we should replace architecture
% with either tool or infrastructure or something else...}\mrnote{DCI,
% maybe?}\mtnote{done} \mrnote{I am really not sold on the claims made in
% the above paragraph and then it is not really helped by the below
% paragraph. I can see maybe you don't call C and C a logical component,
% but I would like an example of a cross- machine pilot job implementation
% in which there is no way to communicate or coordinate with any pilots or
% tasks.}\mtnote{There might be a misunderstanding: communication and
% coordination are necessary between the WM and TE. Simply, C\&C can be
% achieved, organized and supported at implementation as preferred.
% Namely, they way to achieve, organize and support C\&C is *not* part of
% what defines the reference model for the implementation of a pilot
% system.} \mrnote{The exact definitions given in task dispatching are
% actually C and C, just not called that.} \mtnote{Again, there might be a
% misunderstanding: task dispatching requires communciation of data both
% relative to the task and to the coordination system used to agree on how
% to dispatch a task. C\&C is not task dispatch in itself.}\mrnote{I am
% really uncomfortable with the way it is presented as not integral, and I
% think we should table this discussion again.}\mtnote{I rewrote the whole
% paragraph including the clarifications given to your previous comments.
% Any better?}

% \jhanote{the paragraph started talking about C\&C not being integral,
% but then focusses/ends on M-W}\mtnote{Rewritten. Any better?}

% \mrnote{This paragraph is making it somewhat difficult
% to understand. A single super computer or machine is not a 'distributed
% computing infrastructure.' Therefore, the assumption when I am reading
% this is that you may be referring to a grid. But are you really talking
% about a machine? Because if you're talking about a grid, then you know,
% submitting to the DCI would mean submitting to a grid-wide scheduler,
% not a machine-specific scheduler. It should be clarified.}\mtnote{I
% clarified the relationship n:1 between resources and DCI. Any better?
% Please note that this creates an inconsistency between how a resource is
% defined and how the same name is used to indicate a cluster within a
% DCI. This is why, the common use of both the terms DCI and resource are
% inconsistent and why here I had tried to redefine them consistently.}

% The tasks of a workload are bound to one or more Task Executor that in
% turn is bound to one or more pilots. In this way, tasks do not have to
% be directly scheduled on the job management systems of any the DCI's
% resources --- would such system be exposed by a single cluster of the
% DCI or by a DCI-wide scheduler. Furthermore, the binding of tasks to a
% pilot might happen before or after the pilot has been assigned a portion
% of any of the DCI's resource, and it can last for the whole time the
% pilot can hold on its resources. This execution strategy requires
% dedicated functionalities both to perform the binding of tasks to pilots
% but also to provision pilots so that they can be assigned portions of
% any of a DCI's resource.
...
% \jhanote{need to justify the above sentence.}\mrnote{Maybe
% we can say that we will now motivate   why we believe the two core
% functionalities are..., etc. I agree this sounds like a strong claim,
% maybe the conclusion of the paragraph, not the lead.} \mtnote{Rewritten.
% Any better?}
...
% \mrnote{Should this be a citation? It just sounds weird when it says
% ``the one''}\mtnote{I am not sure I udnersatnd why we would need a
% citation. I changed `the one'. Any better?}
...
% \mrnote{Might not need the descriptive clause. We've covered
% that already above.} \mtnote{I will decide later, when the final
% version will be available.}

\begin{description}

% \item[Application.] A software that enable the user to achieve a set of
% goals.\mtnote{This was not a deifnition. It says what an applicaiton
% does, not what it is. I adjusted the definition. I am still not sure
% why do we need this definition at all.}
...
% \jhanote{I think this entry should be
% removed from table 1, just like application does not find a mention in
% table 1?}\mtnote{Done.}
% \jhanote{MR suggests ``and associated logic'' along with ``possibly
% correlated''}\mtnote{I do not understand this note.}
% \mrnote{We should not use the term workload, as in the definition of
% Application, before it is defined. But then application is used
% circularly around to itself... not sure how to fix that, but we can't
% expect someone to know what 'responsible for the definition of the
% workload' means when they don't know what a workload is}\mtnote{Also in
% this case, I do not undersatnd this note.}

\item[Task.] A set of operations encoded so that they can be executed
on the resources of a DCI. Implementations of a task may include
wrappers, applications, scripts, and, more in general, any type of
`application kernel'.

% \mrnote{(i) My issue here is, a single workload is a set of tasks.}
% \mtnote{in an arbitrary complex set of relations.} \mrnote{ An
% application, by these definitions, should be comprised of a possible
% series of workloads, even though workload is used in the singular to
% define an application. (ii) On the note of consumers, I find the use of
% the phrase 'pilot job consumer', when not previously discussed or
% defined in this manner, unclear. At this point, you haven't defined what
% it means to be a Pilot Job system consumer. It is unclear in what way an
% Application 'consumes' PJ resources (by definition, not by my
% knowledge)}\mtnote{This note is probably outdated. It shows thought a
% problem that I still do have with how we use application and workload
% within radical.}
% \mrnote{Also, the use of the word program - is this all-encompassing,
% i.e. do you mean program binaries and programs? Or do you consider a
% binary a program? You know, if you consider a program, a series of
% instructions for a computer. It's just the word is a little iffy. I
% would adopt what BigJob uses which is 'application kernel.' or say
% programs and/or binaries}\mtnote{Done. Any better?}
...
% \msnote{I dont think
% a job is actually a container of executables}\mtnote{Any suggestion on what a
% job should be for you?} \msnote{Should we maybe relate job to tasks? Although I
% guess we do that in the next paragraph already ...}\mtnote{I would avoid to mix
% the two but happy to hear your reasoning about it.}
...
% \msnote{Although I probably agree, doesnt this "note" add more confusion than it
% resolves?}\mtnote{The goal of the note is to reassure the reader that knows
% about jobs and tasks but uses it interchangeably. Happy to take it away if both
% of you find it confusing.}
...
% \msnote{I dont understand the part of "the sum" and the following
% example ...}\mtnote{Any better? Please feel free to edit the terminology
% used in the example if you think editing is needed.}

% \mrnote{I'm uncomfortable with the word programs again,
% especially for a 'pilot'.}\mtnote{I substituted 'program' with
% executable. As usual, please indicate the preferred word if you do not
% feel comfortable with 'executable'.} \mrnote{Later we say the term pilot
% = the term agent = the term pilotjob. Not sure if I can agree with that
% - the PilotJob (or Pilot) is just a job (or container) you submit to the
% queue. It's not a running program. Just a job, traditional job
% script.}\mtnote{Done.} \mrnote{The 'agent' in this case is the 'program'
% that becomes active when the 'job' has become active in the batch queue.
% Remember that earlier we define a job and a task as being two separate
% things (this is at ``CU'' level). But we lump them together here by
% saying that the job (Pilot) is the task (program / set of operations
% that will be performed). Later, binding is discussed. Early binding is
% defined as binding of tasks to inactive pilot. So, if pilot = program,
...
% \msnote{I think I start to understand the earlier definition of
% "container sum", but we might want to clarify that earlier}\mtnote{I
% have attempted an early clarification. Please let me know whether you
...
% \msnote{Im not sure if it is more reliable, but we can add or replace
% with "flexible"?}\mtnote{Done.}
...
and tasks. A portion of the resources of a DCI is scheduled to a pilot
and one or more tasks is scheduled to a pilot. This is an important
feature of \pilotjob systems because it allows for a task to be bound to
a pilot before of after it is in turn bound to the resources. Binding
pilots to their resources and tasks to pilots depends on placement
choices. As such, the binding process in itself is often an instance of
scheduling.

% \msnote{The "first and then" is possibly in conflict with the next
% sentences.}\mtnote{Done. It is different from how we have been
% discussing multi-level scheduling and binding until now. Please let me
% know whether both of you are fine with the new interpretation.}
...
% \msnote{The "specified" part here probably requires some
% clearification.}\mtnote{The paragraph reads fine to me but I would be
% happy to add more information. Do you have more details about what it
% should be clarified when speaking about specifying?}

% \jhanote{I would remove the definition of application. It is one of
%   those conundrums, that you'll hardly help anyone, but definitely
%   aggravate someone!}\jhanote{Matteo: this section is ready for
%   clean-up, ie remove comments to the end and possibly into a
%   different file}

%------------------------------------------------------------------------------
% SECTION 4
%------------------------------------------------------------------------------
\section{Pilot-Job Systems: Analysis and Implementations}
\label{sec:4}

Section \S\ref{sec:3} offered two main contributions: a minimal description
...
PS Merriam-Webster agrees}\mtnote{Done for 4.1.}
...
\item \textbf{Pilot Characteristics}: In~\S\ref{sec:3} pilots
  ...
  \msnote{Possibly try to split this in more concrete properties to make it
  easier to use in 4.2. To be decided once MS does more of the pilot section in
  4.2}
...
\item \textbf{Task Binding Characteristics}: One of the core
  ...
  \S\ref{sec:3}, \pilotjob systems may allow for two main types of
...
\item \textbf{Deployment Strategies}: Once the tasks are dispatched to
  ...
  pilots.
  \msnote{Maybe this is more about task execution?}
...
\ref{sec:3}, \pilotjob systems support many different use cases involving
diversified usage modes - e.g. HPC, HTC and Cloud.
\msnote{This can go if we make sure that there is a "context" paragraph in
every 4.2 subsection}
...
\end{table*}
 ...
 \jhanote{I think the set of core properties are fine. There may be a
   need to refine the discussion of each property, but that can be
   deferred for now.}  \jhanote{Based upon the empirical knowledge
   acquired during saga-pilot implementation, is there some thing we
   can add to auxilary properties}
...
HTCondor \textbf{pool}.
%A pool consists of multiple loosely coupled, usually distributed  components
%that together implement the functionality of Workload Manager and Task Executor
%in a PilotJob system.
...
BigJob was designed as a flexible and extensible \pilotjob to work on a variety
of infrastructures.
It was designed to natively support many scientific applications, including MPI
jobs~\cite{saga_bigjob_condor_cloud}.

BigJob supports the basic functionality required of a \pilotjob, with the added
capability of interfacing with a number of different batch queuing systems and
infrastructures (grids, clouds, clusters, etc.).
BigJob was also designed to support MPI jobs without adding additional
configuration requirements to the end-user.
Lastly, the application-level programmability that BigJob offers was
...
%DIANE utilizes a single hierarchy of worker agents as well as a PJ
%\textit{manager} referred to as \texttt{RunMaster}.
%The manager creates placeholders called \texttt{Workers}, which register with
%and receive jobs from the manager.
...
%The implementation of GSI on TCP-level is possible, but currently not yet
%implemented.
...
% state clearly how section 3 and 4 help us define what a pilot is,   the
% necessary and sufficient conditions (if possible), discuss the
...
% tools/sustainability/etc. At some point, we discuss how/why pilots   are
% more than just pilots, eg can be RM layer for middleware,   runtime
% framework etc}\mtnote{I have organized the following items on the lines
% you have traced, with some changes in the order and number of the
% items.}

Sections~\ref{sec:3} and~\ref{sec:4} offered respectively a description of the
minimal capabilities and properties of a \pilot \mtnote{In Section 3 we use
\pilotjob. We will have to move to \pilot or decide to use the less correct
\pilotjob here too.} system; a vocabulary defining `pilot' and its cognate
concepts; a classification of the core and auxiliary properties of a
paradigmatic pilot system implementation; and the analysis of an exemplar set
of pilot system implementations. Considered altogether, these contributions
show how the notion of \pilot indicates a paradigm for systems taking care of
executing tasks on (distributed) resources.

The \pilot paradigm identifies a specific pattern of middleware system with
unique characteristics. Middleware because it operates above operating systems
and, in case, other middleware interfaces, unique because it is defined by
resource placeholding \mtnote{we use resource overlay in 3.1 but we should
probably use placeholding seeing how we use overlays these days in OWMS},
multi-level scheduling, and early/late binding. Every software system with
these three characteristics, built upon the logical components and the
functionalities described in Section~\ref{subsec:3.1} is an implementation of a
\pilot system as seen in Section~\ref{sec:4}.

\jhanote{Need thinking: Following on from discussion yesterday, is
  early/late binding a logical consequence of multi-level scheduling?
  Can we absorb the latter into the former?}

It should be noted that the \pilot paradigm is also general. While the
operations of a task performed by means of a \pilot are usually computational
in nature, in principle, they might also operate on other type of resources
such as data or networking, depending on the resources made available by an
infrastructure to its users. In this context, it would be likely for some
\pilotjob systems to use the terms `pilot data' or `pilot network'. In Ref.
[pilot data], the notion of \pilotdata has been conceived using the power of
symmetry, i.e., \pilotdata taken to be a notion as fundamental to dynamic data
placement and scheduling as \pilotjobs is to computational tasks.

The unique characteristics and generality of the \pilot paradigm has been
modeled in Ref.~\cite{Luckow:2008la}. This investigation was motivated by the
desire to provide a single conceptual framework -- the so called $P*$ Model --
that would be used to subsume the design characteristics of the various
\pilotjob implementations that exist, a sample of which have been analysed in
detail in Section~\ref{sec:4}. \mtnote{Is this still the case? I think this
has become one of the goals of this paper. How do you want to mention P* here?
...
the model led to the generalization of the model as the {\it P* Model of Pilot
Abstractions}.
...
% distributed system paradigms. What `other' paradigms should we consider? In what
% sense the pilot paradigm is distributed? Why should we define the \pilot
% paradigm an abstraction?

\subsection{Myth Revisited}
\label{sec:5.1}

\begin{enumerate}

\item PJs are just for HTC and they just circumvent job queuing delays; PJs
unfairly game HPC queuing system;

\item PJs are such a simple concept, it doesn't need more attention:
Conversely, everyone should write their own PJ just because they can;

\item PJs have to be tied to specific infrastructure and infrastructures have
to be tied to specific pjs;

\item PJs are stand-alone tools passive (system) tools, as opposed to user-
space, active and extensible components of a CI;

\item PJs have well defined semantics and model. PJs dont help with data
intensive applications.

\end{enumerate}

The pilot paradigm alongside its abstractions offer the opportunity to overcome
a common [cit? cit?] interpretation of \pilot systems as...


\subsection{Pilot and Middleware Layer}
\label{sec:5.2}

The definition of the term `pilot' and the critical review of its
implementations highlight the relation between \pilot systems and middleware
as commonly defined in [cit, cit, cit]. The notion of pilot as a resource
placeholder is something more than a job as defined in the context of Grid-
based middleware. For example, cloud-based DCIs \mtnote{Use of DCIs for plural
needs to be consistent across the paper.} introduce notable exceptions and
differences in the way in which pilots can be provisioned.[cit, cit] Within a
IaaS [cit], Virtual Machines (VMs) and not jobs are used for their
provisioning. VMs can often be instantiated without waiting into a queue  with
a predictable instantiation time, usually minimal wnen compared to the queuing
time often experinced on Grid-based DCIs [red Vishal paper]. Furthermore, the
limitations on the execution time of a VM can be virtually absent, a
limitation that on grid-based middleware is imposed by the queue walltime.
Clearly, overheads are introduced by having to deal with VMs and not simple
jobs and the model adopted within a IaaS-based DCI to assign resources to each
VM can affect the flexibility of the whole \pilotjob system. [cit] A similar
assessment could be done for a DCI deploying a PaaS model of cloud
computing.[cit]

% \mrnote{Though you avoid queue wait, you introduce VM start-up
% time overhead as well}


\subsection{Pilot and Application Layer}
\label{sec:5.3}

The current state of workflow (WF) systems~\cite{nsf-workflow,1196459}
provides a motivating example for the P* Model and the Pilot-API: even though
many WF systems exist (with significant duplicated effort), they provide
limited means for extensibility and interoperability.  We are not naive enough
to suggest a single reason, but assert that one important contributing fact is
the lack of the right interface abstractions upon which to construct workflow
systems; had those been available, many/most WF engines would have likely
utilized them (or parts thereof), instead of proprietary solutions.
Significant effort has been invested towards WF interoperability at different
levels -- if nothing else, providing post-facto justification of its
importance. The impact of missing interface abstractions on the WF world can
be seen through the consequences of their absence: WF interoperability remains
difficult if not infeasible. The Pilot-API in conjunction with the P* Model
aims to prevent similar situation for \pilotjobs.\mtnote{To be abstracted from
workflow to application.}

\subsection{Pilot and Security}
\label{sec:5.4}

\mtnote{I know something about security but I do not feel qualified for
writing even a superficial analysis of the security implications of the pilot
paradigm :)}

\subsection{Pilot and the Enterprise}
\label{sec:5.5}

\mtnote{Waiting for comments/text from AndreL.}

\begin{itemize}

% \item \textbf{Pilot abstraction} as a well-defined, independent
% distributed system abstraction. \jhanote{state clearly how section 3 and
% 4 help us define what a pilot is, the necessary and sufficient
% conditions (if possible), discuss the classifiers and apply/discuss them
% to the set of pilot systems here.} ``pilot-abstractions works!'' , p* as
% a model ok. Our initial investigation~\cite{Luckow:2008la} into \pilot-
% Abstractions was motivated by the desire to provide a single conceptual
% framework --- referred to as the P* Model, that would be used to
% understand and reason the plethora and myriad \pilotjob implementations
% that exist.

% \item \textbf{Generality of the pilot abstraction} when compared to different
% types of resources. Is pilot useful only for compute resources? What about pilot
% data? Pilot network? Other types but compute, data, network?
...
% evolution in the multiple implementations we introduced and described in
% Section 4?

% \item \textbf{Relationship between pilot abstraction and different types
% of middleware}: grids, cloud, HPC.

% \item \textbf{Pilot abstraction and `lessons for the workflow systems'}.

\item \textbf{Take home messages}:
...
\item \textbf{Sociotechnical}. \jhanote{What is the future of PJ?};
...
\item \textbf{Engineering}. \jhanote{Why should we do to enhance the
...
\item \textbf{Contributions}. \jhanote{(i) we provide first
comprehensive historical and technical analysis, (ii) set the stage for
a common conceptual model and implementation framework, and (iii)
provide insight and lessons for other tools and higher-level frameworks,
such as Workflow systems possibly}
