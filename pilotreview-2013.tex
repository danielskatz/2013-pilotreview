...
definition(s). } 
\amnote{I absolutely agree with the definition (early binding:
    scheduling tasks to a not-yet-active pilot; late binding: schedule
    tasks to an active pilot).  What puzzles me is the motivation used
    for late binding (high throughput of tasks).  I may have a
    different definition of 'throughput' in mind though: for me this
    refers to tasks/second.  Both a pilot used with early and a pilot
    used with late binding can be very efficient, and achieve the same
    throughput of tasks.  In fact, for a single pilot (and an ideal
    pilot system implementation), there should be no performance
    difference in early vs. late binding.  It is only in the
    multi-pilot case where it makes a difference -- but not in
    throughput, but in total TTC.  I thought this is also what your
    experiments showed?  If that is something your data disagree with,
    then please disregard, and feel free to remove that discussion as
    'solved' :)}
