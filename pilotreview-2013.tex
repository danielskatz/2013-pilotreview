...
both in the functionalities and the vocabulary adopted by
different \pilotjob systems. Often, implementation details hide the
...
This Section offers an analysis of the architectural components, functionalities, and terminology shared by every \pilotjob system. The goal is to offer both a conceptual description of an exemplar \pilotjob system and a well-defined vocabulary that can be leveraged to produce a comparative and critical analysis of diverse \pilotjobs frameworks in \S\ref{sec:4} and \S\ref{sec:5}.
...
is based on two separate, but intercommunicating logical components: a \textbf{
Workload Manager} and a \textbf{Task Executor}. The Workload Manager is is
related to the management of the workloads and their tasks; the Task Executor
to the execution of the tasks on target resources. As will be shown in
\S\ref{sec:4}, the implementations of both components significantly vary from
...
% \alnote{is data staging minimal?}\mtnote{if you think about it as
% input/output files not strictly - even if basically you would be able to run
% very little science without them. When you consider staging of data in
% general, then I believe it becomes minimal.}\mrnote{What do you mean by
% considering it in general? I could stage my files manually to-and-fro and
% only worry about the Pilots as task executors using hardcoded paths to my
% data. Data-staging therefore would not be required, especially on machines
% with shared filesystems}\mtnote{Apologies, my comment was too generic. I
% mean that it is not only about staging I/O files, it can also be about
% staging the pilot/agent --- BJ auto-install into a venv for example --- or
% downloading the the executable used for the task. If we take away the very
% possibility to move data --- whatever function they play into the execution
% `model' of PJ --- in and out I think that the PJ system as a whole cannot
% work. If you think this explanation is convincing, we might want to
% transform it into a small paragraph.}
...
The term `pilot' is often associated with that of `placeholder'. This is a way
...
executing tasks submitted (to the system that the pilot belongs) that will
utilize them. Thanks to these capabilities, a pilot allows for a user or a
group of users, depending on implementation details, to gain exclusive control
over the binding and execution of her or their tasks.
...
\subsection{Comparing \pilotjobs via Classifiers}
...
of \pilotjob systems.  While these classifiers are by no means meant to be
...
categories: \textit{Resource Provisioning}, \textit{Workload Execution and
...
Resource interaction classifiers capture how a \pilotjob system interacts with
...
Due to its absence in some \pilotjob systems, data handling and management
...
As non-functional properties we define the properties that are not specifically
related to \pilotjob systems but rather describe general concepts commonly found
...
\item \textit{Authentication and Authorization}: How is a user identified in
the system and how are identities and access credentials delegated to individual
resources / pilots?

\item \textit{Transfer Protocols}: What network protocols that are used to
connect the individual component of the \pilotjob system?
...
framework. \jhanote{Which is the general concept? I think
  Pilot-Manager is the general concept and BigJob-Manager a specific
