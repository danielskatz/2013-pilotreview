...
framework. This not only did not prevent duplication of development effort
across frameworks and projects but also hindered the appreciation for the
generality of the \pilot abstraction and the underlying theoretical framework
to the \pilot systems and the application execution paradigm they enable.
...
RADICAL-pilot has be used to execute [...] milion jobs on XSEDE.
\msnote{This sounds biased. I would think there are better examples of
production usage than that of RP ...}
...
on multiple levels.} Analogously, the failure of a \pilot among the many
...
adoption. As with other middleware [cit] the problem is not to eliminate
special purpose systems in favor of a single encompassing solution but it is,
instead, having both of them, depending on the application and use case
...
more effective.\msnote{The previous sentence might undercut our story more than
we wish, as all existing systems started as that, but consequently ran into
trouble when trying to grow behond that} Another rationale against rigid
