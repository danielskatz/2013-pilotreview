...
% Execution model.
\mtnote{TODO: Explicitly describe execution model of DIANE.}
...
(CU) Manager, and an Agent. The Pilot Manager allows to describe pilots
and then submit them to DCR, while The CU manager to describe tasks (i.e. CU)
and to schedule them to one or more pilots. The Agent is executed on the DCR and
executes the CUs pushed by the CU manager.
...
\mtnote{A merging with auxiliary properties or each pilot system implementation
will happen.}
...
% \pilots can benefit from the more predictable VM instantiation time because
% VMs can be used as resource placeholders. \jhanote{I don't think this is the
% message we want to convey: viz., pilot help exploit predictable execution that
% is fundamental to the DCR} \jhanote{... till here.}
...
% including their data and networking subsystems, and the model adopted within a
% IaaS-based DCI to assign resources to each VM can affect the flexibility of
...
\jhanote{the concept of localization is OK. Can the discussion about
  localization and AAA be merged to save space?} \mtnote{It depends whether we
  want to give the idea that we know that AAA is relevant when we speak about
