...
constituents are task, resource, infrastructure and scheduling.
...
The concept of operation entails that of resources, namely limited,
typed and physical quantities that are consumed by executing the
operations that describe a task. Resources are grouped into so called
'infrastructures', often geographically separated by the users
interested in performing their task. 

The operations of a task associated to a \pilotjob are usually
computational but in principle they might also operate on data or
...
submit to a scheduler on an infrastructure that offers the required
amount and type of resources. Jobs may be described in a variety of
formal languages indicating, among other properties, not only the amount
and type of resources needed to perform its tasks but also the amount of
time that the tasks belonging to that job should take to complete. 
...
collected and made available to the manager. 

\mtnote{Vocabulary: manager}

The manager is responsible for pulling tasks from the task pool and
scheduling them into one or more agents. Managers can offer a vast array
of capabilities and they differer greatly among \pilotjob
...
offered or potentially implementable in pilot managers.

\mtnote{Vocabulary: agent}

Agents run the tasks pushed to them by the manager. Clearly, an agent
must have the right amount and type of resources in order to run its
tasks and it is this requirement that brings together pilots and jobs.

\mtnote{Vocabulary: Pilot Job}

A pilot job is a system that combines all the conceptual constituents of
