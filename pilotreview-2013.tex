...
diverse DCRs. The \panda server is implemented by three main components: a Task
...
or lcg-cp). The Job Dispatcher delivers the tasks to the pilots running on the
bound DCR.
...
% Workload Semantics.
\panda has been designed to serve  specifically the ATLAS use case. As such,
\panda is used to execute mostly single-core. parametrized tasks.\mtnote{Is this
true? I did not find specific literature, so this is mostly an extrapolation of
mine.} Initially, data requirements were assumed for each task in terms of input
and output files. Since its initial design, \panda has been evolving towards a
general purpose workload manager. Experimental support for multi-core tasks is
under development, especially in relation to the development of the JEDI (Job
Execution and Definition Interface) and ATLAS AthenaMP, the multi-core
implementation of the ATLAS software framework.
...
characteristics. As such, \panda offers late binding but not early binding
capabilities.
