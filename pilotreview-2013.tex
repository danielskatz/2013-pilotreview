...
the previous paragraph. Also, I think the clustering is   the reverse of what
is suggested, viz., the properties are   defined/derived, and then pilot-jobs
...
\textit{Core} properties are common to each \pilotjob implementation while the
auxiliaries characterize specific \pilotjob implementations and they are not
shared among all implementations.  More importantly however, auxiliary
properties are not unique to \pilotjob and are properties that can be found in
...
Dispatching, and Task Execution. Pilot provisioning is reviewed by focusing on
the requirements and dependences between the \pilotjob system and the
underlying distributed resources, and on what type of resources the pilot
system exposes in terms of computing, data and networking. For Task
Dispatching, two specific core properties are considered: The semantics of
multiple types of workload and how the tasks of a given workload can be bound
to single or multiple pilots. Finally, task execution is analyzed by reviewing
different strategies for task scheduling and by describing how \pilotjob
systems support specific execution strategies for multiple types of workload.
...
our critical review of PJ implementations?}
