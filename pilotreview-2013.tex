...
not submit a `job' to a cloud middleware. I replace executed by submitted.}
Accordingly, tasks are considered as the functional units of a workload, while
jobs as a way to schedule tasks on a DCR with a specific middleware.
\jhanote{minor nitpic: are we assuming 1 task per job? I don't think we
should..} \mtnote{I do not think so as we use `tasks'. Should we make it
explicit?}  It should be noted that, given their functional equivalence, the two
terms can be adopted interchangeably when considered outside the context of
\pilotjob systems. Indeed, workloads are encoded into jobs when they have to be
directly executed on DCRs that support or require that type of container.
