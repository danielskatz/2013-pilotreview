...
named inconsistently\msnote{Inconsistently could mean here that the systems are
not consistent themselves, but I think we try to convey is that the systems are
not consistent with each other}\mtnote{Inconsistently refers to `named'. I can
move it before `named' but I would leave this decision to a native English
speaker.}, often with the same terms referring to multiple concepts or the same
concept named in different ways.

This section offers an analysis of the architectural components,
functionalities, and terminology shared by every \pilotjob system\msnote{Didnt
we just about established that the terminology is not shared? :-)}\mtnote{we
established that `tend to be' not that `they are always' named inconsistently.
If you feel strongly about it I can delete `, and terminology'}. The goal is to
...
\onote{Sometimes we use 'logical' components and sometimes  'architectural
components' throughout this section and others.  I think logical components
should be used consistently and the  term 'architectural components' should be
avoided all together.}\mtnote{Done.}
...
As seen in \S\ref{ssec:evaluation}\msnote{Do we really want such a forward
reference?}\mtnote{That portion of S2 has been lifted and moved to S4,
reference included. I will wait for the first complete draft of S4 to be
completed before deciding how to alter this portion of the text.}
...
characteristics that are integral to the \pilotjob paradigm\msnote{I like the
concept "pilot-job paradigm", it might be a good "general" term to use} itself.
...
is based on two separate, but intercommunicating\msnote{Is the definition of
two-way on purpose?}\mtnote{Yep.} logical components: a \textbf{ Workload
...
\pilotjob paradigm.\msnote{How about changing the last words into: "... the
\pilotjob paradigm."?}\mtnote{We would repeat `paradigm'. I replaced
`communication and coordination paradigm' with `communication and coordination
pattern'. Apparently, in common English `paradigm' and `pattern' are synonyms.}
...
process when compared to scheduling tasks directly on DCIs. \onote{I am not
sure if I understand the performance part.}\mtnote{Any better?} The tasks of a
workload are bound to one or more Task Executor and, as such, to a pilot,\onote{Shouldn't this be 'one or
more Task Executors'? We haven't introduced  pilots yet.}\mtnote{We mentioned
pilot in S2 and I think we can assume here, and in the following paragraphs,
that the reader has by now a common understanding of the term. I tried to make
the substitution but I have a doubt: Do we think Task Executor is synonym of
pilot? Or is pilot just one of the components of a Task Executor?} without
having to be directly scheduled on the DCI's job management system.
Furthermore, the binding of tasks to a pilot \onote{'Task Executor'?} might
...
so that they can be assigned portions of the DCI's resources.\msnote{Arguably,
the usage of bind/bound in this paragraph comes a bit too early as the notion
has not been developed yet.}\mtnote{My impression is that we can refer to the
common understanding of the term `bind' as we are not using any
specific/technical meaning of the term.}
...
scheduled\msnote{scheduled?}\mtnote{done but bound means both scheduling and
running so I had to add `run'. I choose `run' because `executed' --- that I
think it would be better --- would clash with `for the execution of the
tasks'.} to the DCI resources by means of the DCI capabilities but, once
scheduled and then run, pilots make those resources directly available for the
execution of the tasks of a workload.\msnote{Arent the resources only available
after the pilot becomes "running"?}\mtnote{Agreed. That is what I meant to
imply when using `bound'.} Thanks to resource overlays, \pilot systems can also
...
\msnote{Possibly use typesetting to mark usage of strictly defined terms in
this section?}\mtnote{Done.}
...
infrastructures that support or require that type of container\msnote{You mean
when not using pilots, right?}\mtnote{Yep.}.
...
the terms `pilot' and `pilot-job'\msnote{Now it reads like that the new dci's
have lead to the interchangable use of pilot vs pilot-job, is that what you
want to say?}\mtnote{I don't think so. To my eyes, it reads as: when there are DCIs
that use multiple types of containers, using the name of a specific container
for all the DCIs leads to conceptual confusion.}.
...
turn bound to the resources.\msnote{Isn't this contradictory because of the
"first ... and then"?}\mtnote{I do not see contradiction but I will be happy to
consider an alternative phrasing.}
...
%\mrnote{Best Falkon ref: \url{http://dev.globus.org/wiki/Incubator/Falkon\#FALKON:\_A\_FAST\_AND\_LIGHT-WEIGHT\_TASK\_EXECUTION\_FRAMEWORK}}
\mrnote{WIP: Bullet points that will later be converted to text}

\paragraph{Rationale}
\begin{itemize}
\item Enable the rapid and efficient
execution of many independent jobs large compute clusters
\end{itemize}
...
\paragraph{Design Goals}
\begin{itemize}
\item Reduce task dispatch time by using a streamlined
dispatcher that eliminates support for features such
as multiple queues, priorities, accounting, etc.
\item Use an adaptive provisioner to acquire and/or release
resources as application demand varies
\item Cache data from remote locations to local disk at the
computational resource to achieve better application
...
\paragraph{Applications}
\begin{itemize}
\item fMRI (medical imaging)
\item Montage (astronomy)
\item MolDyn (chemistry, molecular dynamics sims)
\end{itemize}

From Falkon site: Large-scale applications from many domains (e.g., astronomy, medicine, biology, chemistry, molecular dynamics, economics, and analytics) have been successfully executed using the Falkon framework.

\paragraph{Deployment Scenarios}


\paragraph{Resource Landscape (e.g. grid/cloud/hpc/etc)}
Developed for the grid environment - expanded to clouds, Blue Gene/P. 
Work has also been expanded in the data-realm (see: data dispersion)


Currently tested on: TeraGrid, TeraPort, Amazon EC2, IBM 
Blue Gene/L, SiCortex, Workspace Service


\paragraph{Architecture \& Interface}
Two techniques [1] multi-level scheduling and [2] streamlined task dispatching

Falkon 2-tier architecture: 
\begin{itemize}
\item [1] Dispatcher: GRAM4 Web Service accepts task submission and sends to executors
\item [2] Executor: Runs tasks on local resources
\item Provisioner: Static and dynamic resource provisioning
\end{itemize}

Falkon can suspend faulty hosts




\paragraph{Falkon Conclusion}
A Note on Performance: Falkon can achieve relatively high throughput 
in the range of hundreds to thousands of tasks per second when running
fine grained tasks, with a per task overhead measured in milliseconds
throughput in the range of hundredsto thousands of tasks persecond when running fine grain
tasks, with a per task overhead measured in milliseconds.
\mrnote{WIP}
