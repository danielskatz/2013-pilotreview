...
\textbf{Workload Manager}, and a \textbf{Task Executor}. The Overlay
Manager handles the definition, instantiation, and use of one or more
resource overlay --- here called `pilot' --- on single or multiple DCIs.
The workload manager handles the inspection, submission and retrieval of
one or more workload on the available resource overlays. Finally, the
Task Executor takes care of executing the tasks of each workload by
means of the resources offered by the resource overlays.
...
\textbf{task execution}. Every \pilotjob system needs to be able to
schedule \pilotjobs on the targeted resources, schedule tasks on the
available pilots and then leverage these \pilots to execute the tasks of
the given workload. Many more functionalities are needed for example to
implement and enforce security policies, or to manage data staging but,
strictly speaking, all these functionalities are not necessary for
\pilotjobs or tasks to be scheduled and then executed on distributed
resources. As such, these functionalities are here considered
implementation details, depending on the specific characteristics of the
given user, workload, and DCI.
...
The distribution of functionalities among the Overlay Manager, Workload
Manager and the Task Executor depends on the communication patterns and
coordination strategies chosen to implement a \pilotjob system. As such,
a specific communication pattern or coordination strategy  should not be used to define the \pilot
