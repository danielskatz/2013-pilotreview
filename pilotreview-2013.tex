...
% Architecture.
The Coaster System is composed of three main components: a Coaster Client, a
Coaster Service, and a set of Workers. The Coaster Client implements both a
Bootstrap and a Messaging Service while the Coaster Service implement a data
proxy service and a set of job providers for diverse DCRs middleware. Workers
are executed on the DCR compute nodes to bind compute resources and execute the
tasks submitted by the users to the Coaster System.
...
% Execution model.
The execution model of the Coaster System can be summarized in 7 steps: 1. a set
of tasks is submitted by a user via the Coaster Client API; 2. if not already
active, the Bootstrap Service and the Message Service are started by the Coaster
Client; 3. if not already active, a Coaster Service is instantiated for the
DCR(s) indicated in the tasks description; 4. the Coaster Service gets the task
descriptions and analyzes their requirements; 5. the Coaster Service submits one
or more Workers to the target DCR depending on whether any worker is already
active and with free resources; 6. when a Worker becomes active it pulls a task
and, if any, its data dependences from the Coaster Client via the Coaster
Service; 7. the task is executed.

%% 5 core properties
% Pilot Resources.
Each Worker holds compute resources in the form of compute cores. Data are
staged directly from the client to the Worker or via the Coaster Service acting
as a proxy running on the DCR resources. Coasters implements a dedicated
communication protocol among the Coaster components that is used also to
transfer the tasks' input/output data. Networking capabilities are assumed to be
available between the client and service host, and between the service host and
workers nodes.
