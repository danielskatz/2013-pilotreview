...
\begin{abstract} 
  ...
  they are tools/services that support multi-level and/or application-level 
  scheduling by providing a scheduling overlay on top of the system-provided 
  schedulers.
  \aznote{Not sure it is good to open our abstract stating the confusion/uncertainity
    inherent in defining Pilot-Jobs...  perhaps first state our motivation for this
    work (``aim(ing) to provide appropriate context, insight, and analysis of
    a \pilotjobs, and thereby bring about a hitherto missing consilience...'')
    and then state that it's somewhat controversial?  I see where it could
    be eye-catching to outright state that there IS controversy/uncertainity
    though, and that we are aiming to quash it.  Making this note to
    try and figure this out :)}
  Nearly everything else is either specific
  ...
  \pilotjob like capabilities.
  \aznote{Is a ``comprehensive survey'' still true given last week's discussion of focusing on ``landmark'' pilotjobs?}
...
We first present a brief chronological order of the evolution of Pilot Job systems, beginning with simple Master-Worker schemes through advanced workload management systems.

\alnote{The list format is not very readable in my opinion}
\begin{itemize}
    \item Pre-\pilotjob Systems:
...
The Master-Worker scheme was adapted to grids as a means of farming tasks from a master to a various number of workers, and could easily be adapted to run in a platform-independent way across the potentially heterogeneous resources. In the grid computing context, Master-Worker schemes could respond to the dynamically changing resources by adapting the number of workers to match the resource availability. Although Master-Worker schemes were important, they required application modification. 
% Cite: Adaptive Scheduling for Master-Worker Applications on the Computational Grid
% Cite: Condor MW
...
% The development of Condor-G
% Paper: Creating Personal Adaptive Clusters for Managing Scientific Jobs in a Distributed Computing Environment
...
\mrnote{Would like to discuss this and develop for next week}
...
This section explains how \pilotjobs have historically been used, in no particular order.

\pilotjobs are most commonly used for the execution of many tasks through the use of a container job. \pilotjobs are often measured by their throughput, that is, the number of tasks that they can complete per second (tps). As such, \pilotjobs are used to achieve high-throughput, for example, when using genome sequencing techniques or ensemble-based applications. \pilotjobs have also been used for parameter sweeps, chained tasks, and loosely-coupled but distinct tasks. %note to self: cite these with papers

Multi-scale simulations have also benefited from the use of \pilotjobs. A \pilotjob load balancing algorithm was proposed for coupled Multi-Physics MPI Simulations \cite{ko-efficient} in which more processors were assigned to jobs with longer runtimes, so as to make the simulation take the same amount of wall-clock time between points when they need to communicate with one another (via MPI).
% Efficient Runtime Environment for Coupled Multi-Physics Simulations: Dynamic Resource Allocation and Load-Balancing

Some simulations require varying number of tasks to complete -- that is, 10 tasks may start a program but the results of these tasks may spawn more tasks to complete. \pilotjobs can be utilized for these types of dynamic simulations, because once they are active, the agents have an available slot for executing tasks on a given number of cores. As more tasks are added to the system, the \pilot can continue to execute new tasks until the job is considered complete. %Find a citation for this dynamic type of simulation

\pilotjobs have also been used to avoid queue wait times for many jobs as well as harness and utilize different resources (with different batch queueing systems) to do \textit{scale-across} simulations. 
...
\mtnote{why we need a common vocabulary what we do with it.}
...
comparative analysis presented in the next Section.

\mtnote{How we define the common vocabulary.} 

Initially, the concepts used by the implementations of \pilotjob sampled
in \S2 are isolated.  Moving from the general to the particular, both
common and distinctive concepts are first described and then explicitly
defined. As expected, a kernel of general concepts is shared across all
the implementations while less general concepts are specific to one
implementation or another. This is a confirmation that each sampled
implementation should be considered as a particular case of \pilotjob
but also that their differences justify a comparative analysis.

\mtnote{Vocabulary: job}

The terms 'pilot' and 'job' are probably the first two concepts that
need to be defined when looking for a unified vocabulary consistent with
different implementations of \pilotjob. The term 'job' is well
understood and refers to a program that is executed on some computing
resources (refs). No assumptions are made about the type of program or
the type of resources that are consumed.

\mtnote{Vocabulary: queue, scheduler}

In an environment with multiple units of physical or virtualized
resources - e.g. clusters, grids or other distributed infrastructures -
the concept of a job is usually associated to those of queue and
scheduler. A job is submitted to a queueing system and when some
conditions are met, a scheduler takes care of binding some resources to
a job so that it can be executed. 

\mtnote{Vocabulary: binding}

The concept of binding resources to a job is necessary to understand the
meaning of the term pilot in \pilotjob. A common way to bind resources
to a job (refs) involves defining the amount of resources needed to
execute the job before its submission. Once submitted, the job gets
allocated the requested resources, executes its program and
return its output. In such a situation there is a one to one
correspondence between the job and the executed program. Clearly, such a
correspondence depends upon the nature of the program. If the submitted
program is capable of partitioning the resources it got allocated and
then 'pilot' the execution of other programs with those resources, the
binding of resources so obtained can happen dynamically, i.e. at run time.
The former type of binding is called early-binding while the latter
late-binding and when a job is capable of supporting late-binding, it is
called a 'pilot'.
...
\subsubsection{AppLeS - First-Generation PJ-Systems / Master/Worker frameworks}
...
\aznote{Not seeing where a placeholder/pilot reservation is...  it appears
that the schedule is executed directly on target hardware without Pilots?}
...
\aznote{Didn't pick Condor-G/Glide-in, as GlideinWMS uses Condor as well..}
...
\aznote{No mapping vocab for the integrator as of now.}
