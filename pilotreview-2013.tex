...
The \pilot paradigm identifies a type of middleware system with general
and unique characteristics. A \pilot system is a middleware because it
operates above the operating systems and, in case, other middleware
interfaces exposed by the target DCIs. The \pilot paradigm, of which a
\pilot system is an implementation, is general because, in principle is
not dependent on a single type of workload, a specific infrastructure,
or a unique performance metric. The \pilot paradigm is also unique
because it is defined by resource placeholding, multi-level scheduling,
and early or late binding. A \pilot system does not offer just an
alternative implementation of a scheduler, of a workload manager, of a
batch system, or of any other middleware component. A \pilot system
implements a specific patterns of workload execution based on
multi-level scheduling and distributed task execution. Every software
...
analyzed in detail in Section~\ref{sec:4}. \mtnote{Is this still the
