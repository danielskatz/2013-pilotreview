...
of individual instantiations of pilots. \jhanote{the last sentence is a very
  important sentence. It could do with italicising or even boldfacing. Should we
  change the ``implementations'' to just systems, as currently the reader might
  be confused between implementations and individual instantiations?}
...
\item \textbf{Workload Semantics}. The tasks of a workload are dispatched to
  ...
  degrees of semantic richness of workloads and its tasks. The (standard)
  ...
  \jhanote{The list of dependencies was not complete, hence the conversion from
    ...
    compute resources, as it would be unclear to the reader.}
...
implementation and usability of the core properties.  Examples of such auxiliary
properties that might be necessary for a \pilot implementation but that, in of
themselves, would not distinguish a \pilot as a unique system. For example,
...
% authentication, authorization, and accounting; support for multiple usage modes
% like HPC or HTC; or robustness in terms of fault-tolerance and
% high-availability; are all examples of properties that might be necessary for a
...
\jhanote{the transition to 4.1.2 is a bit unsmooth, e.g., no text before we
  ...
  into a 4.1.1 component and a 4.1.2 component?}
...
\item \textbf{Architecture}. \pilot systems may be implemented by means of
  ...
  they affect the Core Properties.  \jhanote{sentence needs attention: plurality
    of ``choice'' seems inconsistent?}  \jhanote{lower case for core properties?
    either way, we should be consistent, e.g., previous paragraph we use l.c.}
...
\item \textbf{Communication and Coordination}. Communication and coordination
  ...
  distinguishing implementation properties. \jhanote{I would not use the word
    distinguishing in the last sentence. OK?}
...
% aux because: its not directly coupled to one of the components/functions
\item \textbf{Security}. \jhanote{We need a sentence that speaks to what are the
    primary consideration before we go exclusionary. How about something like?
    ``\pilot functionality are not defined by the type and method of security
    associated with them; the usage and applicability is however influenced by
    the security''} The properties of \pilot systems related to security would
  require a dedicated analysis. The analysis here is limited to authentication,
...
\jhanote{logical?} \mtnote{I am not sure we need to distinguish among types of
  ...
  explicitly calling them as logical}
...
pools.  It was developed by by the Center for High Throughput Computing at the
...
\mtnote{That uses as little resources as possible to execute the given
  ...
  and emphasising what it controls actively?}
...
tasks to Schedds, and a \MW tool is available to design applications with a \MW
execution pattern. \jhanote{reference to DAGMan needed}\mtnote{I still have to
...
\jhanote{is this match-maker the ClassAds, or is there another level of
  ...
  caution, ok to use a reference to ClassAds?}
