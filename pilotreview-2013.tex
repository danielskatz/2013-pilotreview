...
implementations. Finally,~\S\ref{sec:discussion} closes the paper by outlining the
...
Computing Infrastructures (DCIs) such as Grids, Clouds, or HPC facilities. This
...
considered, for example, energy efficiency, data transfer minimisation, scale
...
\vocab{Task Executor}. The Pilot Manager handles the description, instantiation,
...
workloads on the available resource placeholders. Finally, the Task Executor
takes care of executing the tasks of each workload by means of the resources
held by the placeholders.  \jhanote{does the task executor just execute, or does
  it have some decision making capacity? if the latter, then purely for naming
  consistency, should we consider calling the third component ``task manager''
  or ``Task execution manager'' ? Also we go on to discuss task dispatching and
  execution as functionalities, which leaves current name a bit limiting.}
...
integrated into the Managers and Task Executor. Nevertheless, the Pilot and
Workload Managers and the Task Executor can always be distinguished across
...
example here? In a way it will cut short what we do in S4} \mtnote{I see hwat
...
the worker with the Task Executor, the functionalities related to task
...
will be implemented into the Task Executor. Alternative coordination
strategies, for example where a Task Executor directly coordinates the
task scheduling, might require a functionally simpler Workload Manager but a
comparatively feature-rich Task Executor. The former would require capabilities
...
\begin{table*}
 ...
 \caption{\textbf{Mapping of the core terminology of \pilot systems into the
     ...
     we say common sense? We are defining the vocabulary and the logical model
     so I would assume we are fairly unbounded in our mapping choices.}
...
be executed on a \pilot without waiting in the queueing system of the given
...
the \pilot is still queued or busy.\amnote{what does 'busy' mean in early
    binding?} Late binding is critical in assuring the
aforementioned high throughput by allowing task execution without additional
queuing time or container instantiation time.\amnote{that gives fast startup of
    tasks, but not high throughput.  Also, the pilot must start up, late binding
    won't change thart.  I know we can pick the first pilot which becomes active
    -- but that is not what is being said here...} It should be noted that some
aspects of early binding can also be achieved without a \pilot,\amnote{I don't
    understand what can be achieved w/o a pilot, and what 'both' means in the
    next sentence.} but the
important thing to realize is that the \pilot paradigm allows to do both, even
within the confinements of a single workload. \msnote{I would think parts of
this paragraph also have their place in S5 (if its not already there)}
...
component, some \pilot systems allow for \pilots to be specified by taking into
consideration the properties of (the tasks of) a workload. This type of
specification should not be confused with early binding as the latter requires
for a \pilot to have been already bound to a resource. \msnote{I think this
paragraph is orthogonal to what we write earlier. I suggest to remove this
paragraph altogether.}\amnote{+1, this confuses more than it clarifies...}
...
\begin{figure}[t]
    ...
    \caption{Diagrammatic representation of the logical components,
             ...
             \amnote{the application also defines the pilots, right?}
             \amnote{what is red vs. black; bold lines vs. non-bold?}
...
\jhanote{Given that we've introduced Figure 4, a paragraph that summarised this
  section by way of explaining Figure 4 would be useful to the reader as well as
  provide a soft transition to next section. Thoughts?}
...
This section \jhanote{do we mean subsection or section? also, we should be
  consistent with UC or LC for section and subsection. FWIW, I prefer LC when
  saying ``this section'' but upper case when we reference a section,
  i.e. Section 2.} defines and analyzes \jhanote{I don't think we define, we
  analyze..?} the properties of diverse implementations of a \pilot system. Two
sets of properties are introduced: core and auxiliary (see
...
\amnote{I don't think this list is 'minimal and complete', there are
    many properties of any software which are 'core' in the definition
    given above.  Programming language.  Code license.  Year of
    implementation.  Those are properties needed to implement one or
    more functionalities, obviously.  I guess that those (and other)
    properties are not interesting in this context?  Can you motivate
    why the chosen properties are intersting?  Possibly they mostly
    apply *only* to pilot systems?}
...
\item \textbf{Workload Semantics}. The tasks of a workload are dispatched to
  ...
  \amnote{why are task dependencies (which seems to be what you call
      'workload semantics') a core property?  A pilot system is still
      a pilot system if it does not implement task dependencies (ahem!
      ;-).Ú}
...
the number of \pilot systems should be reduced, it does not support the idea of
one solution should fits all. As with other types of middleware software [cit]
the problem is not to eliminate special purpose systems in favor of a single
