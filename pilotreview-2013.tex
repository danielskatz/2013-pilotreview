...
The goal of this Section is twofold.  Initially, the set of functionalities
presented in \S\ref{sec:vocab} are used as the basis to infer \jhanote{infer?
deduce? discern?}\mtnote{done.} a set of core implementation properties. A set
of auxiliary properties is also defined when useful for a critical comparison
among different \pilotjob implementations. Subsequently, several \pilotjob
implementations are analyzed and then clustered around the properties
previously defined. In this way, insight is offered about how to choose a
\pilotjob system based on functional requirements, how \pilotjob systems are
...
\jhanote{I observe, we do not discuss why ``task binding characteristics''. In
conjunction, with discussion in 4.1.1, I believe reader is left unsure why this
is fit for inclusion as a core property. Please see specific comment in
4.1.1.}\mtnote{We had a discussion of binding but very implicit and limited to
the spatial dimension. I have now extended the explanation adding the temporal
dimension and mentioning binding explicitly.}
...
\pilotjob systems implementations, as well as address/set/provide constraints
on their usability. \jhanote{See comment in the next paaragraph} Programming
and user interfaces; interoperability across differing middleware and other
...
properties that might characterize a \pilotjob implementation but in of
themself, would not distinguish a \pilotjob as a unique \jhanote{would unique
be more appropriate that specific?} \mtnote{Done.} type of distributed system.
...
Section 2? Once decided, I would add an   internal reference after mentioning
the use cases.}. \jhanote{Do we   need to distinguish usability versus use-
...
discussion}\mtnote{I need to think too, happy to discuss about this one.} For
...
varies vastly across implementations. For example, some \pilotjob systems
...
If agree, then   we should use another example}\mtnote{Done.} Analogously, every \pilotjob
systems, support the execution of one or more type of workload but they differ
when considering execution modalities that maximize throughput (HTC), computing
(HPC) or container-based high scalability (Cloud).
