...
community and is often cited as the project that coined the term \pilotjob. But
...
\textit{hodgepodge} of concepts, components and services that have been grown
and curated opportunistically for the past 20 years.
...
more resources is called an HTCondor \textbf{pool}. A pool consists of
multiple loosely coupled components that together implement the functionality of
Workload Manager and Task Executor.

In HTCondor, user tasks (\textbf{jobs}) are represented as job description files
and are submitted to a so-called \textbf{agent} (very different usage of the
term) via a command-line tool. An agent is deployed as operating system services
(\texttt{schedd}) on so-called \textit{gateway machines} (user front-ends of an
HTCondor pool) and accepts tasks from multiple users. Agents implement three
different aspects of the overall architecture: (1) the provide persistent
storage for user jobs and (2) they find resource for a task to execute by
contacting the \textbf{matchmaker} and (3) they marshal task execution via a so-
called \textbf{shadow}.
