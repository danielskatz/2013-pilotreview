...
of 'pilot' and 'job' in this type of system. These two well-known [cit]
concepts are complex, meaning that they have logical constituents that
need first to be clarified in order for them to be properly understood.
Starting from the concept of 'job', its most relevant constituents are
'task', 'resource', 'infrastructure' and 'scheduling'.
...
a task. Resources are physically and logically organized into so called
'infrastructures', agglomerates often geographically separated from the
users interested in performing their tasks. 

The operations of a task permed by means of a \pilotjob implementation
are usually computational in nature but, in principle, they might also
operate on other types of resources such as data or networks, depending
on the resources made available by an infrastructure to the users.

\mtnote{Vocabulary: scheduling}

Tasks consume specific type of resources but they do not entail any
notion of resource provisioning. As a consequence, tasks need to be
bound to resources over time, an operation commonly referred to as
'scheduling' [cit, cit]. A scheduler is an application executed on an
infrastructure that performs scheduling by means of dedicated
algorithms.
...
amount and type of resources over an appropriate extent of time.
Therefore, jobs need to be described in a suitable formal language
...
The concept of 'pilot' builds on some of the constituents of 'job' but
includes also those of 'task pool', 'manager' and 'agent'. 

\mtnote{Vocabulary: task pool}

Functionally, a pilot is a system very similar to those deployed within
an infrastructure in order to offer scheduling capabilities. As shown in
Figure x, users submit tasks to a 'task pool', a store where tasks are
collected and made available to a manager. 

\mtnote{Vocabulary: manager}

In this context, a manager is responsible for pulling tasks from a task
pool and scheduling them into one or more agents. Managers can offer a
vast array of capabilities and they greatly differ among \pilotjob
...
performance considerations are typical examples of capabilities already
offered, or potentially implementable, in managers.

\mtnote{Vocabulary: agent}

Agents run the tasks pushed by or pulled from one or more managers.
Clearly, an agent must have the right amount and type of resources for
an appropriate extent of time in order to run its tasks and it is this
requirement that brings together pilots and jobs.

\mtnote{Vocabulary: Pilot Job}

A \pilotjob is a system that combines all the conceptual constituents of
both 'pilot' and 'job' but also add the three distinctive concepts of
'placeholder', 'early binding' and 'late binding'. \alnote{reads like an 
ingredients list.   Would be good to describe interactions as well.}

\mtnote{Vocabulary: Placeholder}
\mtnote{Vocabulary: early binding}
\mtnote{Vocabulary: late binding}
