...
% Co-Pilot provides components for building a
...
The brief description of the many \pilotjob system implementations
...
distinguishing it from other functionality and middleware systems.
...
constraints, each \pilotjob system exhibits characteristics that are
either common or specific to one or more implementations.
%both common or specific to one or more implementations.
...
without a queue-based middleware, a resource placeholder would be executed by
...
for workload description; we propose a necessarily limited workload taxonomy
based upon the orthogonal properties of coupling, dependency and similarity of
tasks.  Workloads comprised of tasks that are independent and effectively
indistinguishable from other tasks are commonly referred to as a Bag-of-Tasks
(BoT). Ensembles (ENS) represent workloads when the collective outcome of the
tasks is relevant (e.g., computing the average property). The tasks that
comprise the workload in turn can have varying degrees and types of coupling;
coupled tasks might have global (synchronous) or local exchanges (asynchronous),
...
We categorize such workloads as coupled ensembles (C-ENS) independent of the
specific details of the coupling between the tasks. A workflow (WF) represents a
workload with arbitrarily complex relationships among the tasks, ranging from
dependencies (e.g., sequential or data) to coupling between the tasks (e.g.,
frequency or volume of exchange).
...
% The tasks of a workload can be executed on a pilot
% without each task individually waiting in the queuing system of the DCR's
% middleware.
In addition to possibly increasing the throughput of the workload
execution, enabling tasks of a workload to be executed on a pilot without each task
individually waiting in the queuing system of the DCR's middleware
% several advantages,
allows the reuse of active pilots to execute multiple workloads. How tasks are
actually scheduled to pilots is a matter of implementation. For example, a
dedicated scheduler could be adopted, or tasks might be directly scheduled to a
pilot by the user.
...
Laboratory at University of Chicago, Computer Science Department. The primary
goal of the Coaster System is to provide \pilot abstraction functionalities to
Swift system~\cite{wilde2011swift,zhao2007swift}. The Coaster System can be used
in a stand-alone modality but, in practice, it should be regarded as a system
specifically tailored for the Swift System requirements and design.
...
Service while the Coaster Service implement a data proxy service and a set of
...
Manager, the Coaster Service and Pilot Manager, and each Worker a Task Manager.
...
Coaster Client API; 2. if not already active, the Bootstrap Service and the
Message Service are started by the Coaster Client; 3. if not already active, a
Coaster Service is instantiated for the DCR(s) indicated in the tasks
description; 4. the Coaster Service gets the task descriptions and analyzes
their requirements; 5. the Coaster Service submits one or more Workers to the
target DCR depending on whether any worker is already active and with free
resources; 6. when a Worker becomes active it pulls a task and, if any, its data
dependences from the Coaster Client via the Coaster Service; 7. the task is
executed.

%% 5 core properties

% Pilot Resources.
Each Worker holds compute resources in the form of compute cores. Data are
staged from a shared file-system, from the client to the Worker, or via the
Coaster Service acting as a proxy running on the DCR resources. Data are not a
type of resource held by the pilots so the \pilot abstraction is not used to
expose data to the user. Networking capabilities are assumed to be available
between the client and service host, and between the service host and workers
nodes. Coasters implements a dedicated communication protocol among the Coaster
System components that is used also to transfer the tasks' input/output data.
...
submitted by the users and that have to be executed on those pilots. These
parameters are evaluated by a custom pilot deployment algorithm that performs a
walltime overallocation estimated against user-defined parameters, and chooses
the number and sizing of pilots on the base of the target DCR capabilities.
...
tasks to execute implementing in this way late binding of tasks to pilots. It
should be noted that tasks are early bound to DCR as its specification is part
...
% workload binding.
As with DIANE, DIRAC features a task pull model that naturally implements the
...
dedicated to a single user or shared and customized for a whole project. Upon
login, a user is presented with a shell-like environment where to issue commands
...
HTCondor, SGI, or OpenPBS job specification language, depending on the job
submission system installed on the user's virtual cluster; 3. the user submits
the job to the job submission system on the virtual cluster; 4. the Cluster
Builder Agent submit a suitable number of Job Proxies, holding one or more cores
on spanning one or more DCR; 5. when the Job Proxies become active, the
user-submitted job is executed on the resources they hold.

%% 5 core properties
% Pilot Resources
Job Proxies hold compute resources in the form of cores. MyCluster does not
offer any dedicated data subsystem and Job Proxies (i.e. pilots) are not used to
expose data resources to the user. Users are assumed to stage the data required
by the compute tasks directly or by means of the data capabilities exposed by
the job submission system deployed on the virtual cluster. Networking is assumed
to be available among the MyCluster components.
...
the paper with SJ. MS wrote an early draft of parts of Section 2 and 4.  SJ and
MT edited the paper.
