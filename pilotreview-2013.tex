...
To assist the reader, we make use of textual conventions: in \textbf{Bold} are
the terminologies from \S\ref{sec:vocab}, in \textit{Italic} we express the
components of our model and in \texttt{Typewriter} we display terminology from
the respective \pilot system.
...
The central component of DIANE is the \texttt{RunMaster}.
It consists of a \texttt{TaskScheduler} and an \texttt{ApplicationManager}.
...
use or build upon.
Together the \texttt{TaskScheduler} and \texttt{ApplicationManager} implement
the \textit{Workload Manager} component.

The \texttt{TaskScheduler} keeps track of the task entries and is responsible
for \textbf{Binding Tasks} to the \texttt{ApplicationWorkers}.
The implementation of the \texttt{ApplicationManager} is as its name suggest,
...
As the \texttt{ApplicationWorker} asks for \textbf{Tasks} from the
\texttt{TaskScheduler} the natural way of implementing the scheduler is in a
\textbf{Late Binding} approach.
...
and executes the work, thereby being a close resemblance of the \textit{Task
Executor}.
