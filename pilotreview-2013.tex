...
\alnote{should we assign a name to these two
  ...
  the others?}\mtnote{In absence of further comments, for now I have used those terms.}\\---\\
...
dedicated to workload management while the job-wrapper belongs to the component
responsible for task execution. Similar examples can be given for the advanced
\pilotjob systems and for the Pilot-Based frameworks: GlideinWMS offers an
extended set of features when compared to the scheduler and dispatcher of
Nimrod-G, but all those features belong to the same, well-defined and self-
contained architectural component --- i.e. the Workload Manager. The same is
valid for Corral or SWIFT: they both offer evolved functionalities and
interfaces to manage workloads but, as such, they both are part of an
architecture where workload management and task execution belongs to two
distinct components.
...
\onote{job-wrapper seems to be an arbitrary term?}\mtnote{Is the the term used in Section 2. Before using it I checked and found it used in this Nimrod-G paper (http://arxiv.org/ftp/cs/papers/0009/0009021.pdf)}

\onote{Flow interrupted w.r.t., prev. paragraph}\mtnote{I am not sure I got the comment right but, if so, I added a `cap' to the beginning of the following paragraph to avoid a sudden change of topic.}\\---\\
...
tasks, input and output files and related data. While communication and
coordination are two fundamental characteristics of every distributed
architecture, none of their specific paradigms is a distinctive feature of
\pilotjob systems. For example, as seen in \S2.2.2, the \MW paradigm is very
common among \pilotjobs. Functionally, the Master can be identified with the
Workload Manager while the Worker with task Executor. However, the \MW paradigm
usually implies a specific distribution of capabilities between Master and
Worker, and, in some cases, also a communication pattern. Moreover, \MW is
...
\mtnote{AL/all: any suggestions to fill these dots ---
  ...
  as a necessity for a PJ system.''}\mtnote{I have incorporated your comment in the paragraph above. Please note that a closure is still needed as it is not strong enough to argue that \MW is too ambiguous to be necessary. A simple rebuttal would be: make it less ambiguous and you will see that it is indeed necessary because, after all, there is no plausible alternative. I still think this specific paragraph is too thin so please feel free to add/change/edit directly the paragraph.}

\jhanote{echoing what Melissa says, it might be more productive to introduce
  C-C as a sub-system at par with Workload Management and Execution Management
  rather than try to all inclusive/exhaustive}\mtnote{This is still in
  progress. To summarize the checkpoint of our discussion: assigning
  Communication/Coordination to a sub-system would mean to move C-C into an
  architectural component of PJ. In so doing we might incur into an over-
  specification of the minimal PJ architecture. After all, C-C does not
  necessarily need to belong to an independent, well-defined architectural
  component of PJ, it could be a feature implemented in the Workload manager 
  and the task executor.}\\---\\

The architectural properties of \pilotjob systems support a distinctive
execution paradigm with benefits for both the compute performance and for the
task submission process. The tasks of a workload are bound to one or more
pilots without having to be directly scheduled on the DCI's job management
system. Furthermore, the binding of tasks to pilot might happen before or after
the pilot has been assigned its portion of the DCI's resources and it can
progress for the whole time the pilot can hold on them. This execution strategy
requires dedicated capabilities both for performing the binding of tasks to
pilots but also for the provisioning of pilots so that they can be bound 
themselves to portions of the DCI's resources.
