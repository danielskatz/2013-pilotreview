...
Pilot jobs are most commonly used for the execution of many tasks through the use of a container job. Pilot jobs are often measured by their throughput, that is, the number of tasks that they can complete per second (tps). As such, pilot-jobs are used to acheive high-throughput, for example, when using genome sequencing techniques or ensemble-based applications. Pilot jobs have also been used for parameter sweeps, chained tasks, and loosely-coupled but distinct tasks. %note to self: cite these with papers

Some simulations require varying number of tasks to complete -- that is, 10 tasks may start a program but the results of these tasks may spawn more tasks to complete. Pilot-jobs can be utilized for these types of dynamic simulations, because once they are active, the agents have an available slot for executing tasks on a given number of cores. As more tasks are added to the system, the Pilot can continue to execute new tasks until the job is considered complete. %Find a citation for this dynamic type of simulation
