...
with the sentence upto ``;''} \mtnote{I am not sure what we wanted to say with
it either. Any better?}
%One or more logical components may be responsible for
%specific functionalities, both on application as well as machine level;
...
exposed by the targeted \mtnote{Target or targeted?} machines. Resource placeholders are scheduled onto
...
configuration and execution parameters alongside information on the executable \mtnote{Too many execut*. Should we use `code' instead of `executable'? Any better option than `code' to replace `executable'?}
...
\jhanote{Please check my edit/deletion. I did not think the statement and
  ...
  `conceptual friction' for the readers, I would keep the point over-specified.}
...
an administrative domain\}. \mtnote{I used `set of machines' because from now on
we use DCR instead of machine and I think we need to link explicitly the two.
Would you agree?}
...
`application', or `script' \mtnote{If we are going to use `code' instead of `executable' in the previous subsection, I would add `code' here.}. Qualitatively, the term `task' is used when
...
schedule one or more task on a DCR with a specific middleware.  It should be noted that,
...
become available only once the job has become active \mtnote{replace `has become active' with `has been scheduled'?}, and only for
