...
into one or more programs. \jhanote{I don't think a task has to be encoded into
one or more applications. Conversely, a single application can be  comprised of
multiple tasks. } \mtnote{Apologies, I meant programs.} \jhanote{I think we
should try to steer the definition of task towoards the workload sans any
resource or scheduling information.} \mtnote{Done.}

\item[Resource.] Finite, typed and physical quantity utilized when executing 
one or more workloads. Compute cores, data storage space, or bandwidth are all 
examples of resources commonly utilized by running workloads\jhanote{how about 
utilized in lieu of consumed?} \mtnote{Done. I might still have some spurious 
`consume' around the subsection but I will try to clean it up.} \jhanote{Also 
an issue of active versus passive. Does the resource execute the workload, or 
is the workload executed on the  resource. I prefer the latter} \mtnote{I 
agree. I believe that in the narrow meaning we give to resources it should be 
the latter. Does this comment still apply to the revised version of the 
definition? BTW, I am not sure I like the last sentence ending with `pools' 
but I know I like the meaning it should convey.}
...
one or more workload. [cit, cit] Infrastructures can be logically partitioned,
